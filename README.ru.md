...
- 🗂️ Платформа с вкладками-воркспейсами: приложение открывается на экране выбора сессии, где можно поднять несколько независимых вкладок — двухсторонний **Image Compare** и синхронизированный N-изображений **Multi Compare** на данный момент — переключение через Firefox-подобный таб-бар, у каждой вкладки свой стейт, холст и пайплайн экспорта, в планах — новые воркспейсы.
...
- 🔭 Мультилупа: ставьте на одно сравнение сколько угодно луп — у каждой свои зона захвата, положение, размер, цвета и freeze; можно скрывать, восстанавливать и удалять по отдельности, есть авто-назначение цвета для новых экземпляров и live-подсветка пересечения зон захвата при перетаскивании.
- 🖼️ Быстрый интерактивный холст: вертикальный/горизонтальный сплит, плавные GPU-зум и панорамирование, синхронные pan/zoom для обеих сторон, режимы просмотра каналов, сглаженные разделители и оверлеи, мгновенный предпросмотр одного изображения через Space + кнопки мыши.
- 🔀 Вкладка Multi Compare: располагайте и синхронизируйте любое количество изображений в одной сцене, перетаскивайте слоты для перестройки layout'а, свободно меняйте размеры разделителей и открывайте полноэкранный focus-режим для любой ячейки.
...
- 🎨 Глубокая настройка интерфейса: видимость, цвет и толщина разделителей, стилизация лупы, настройка текстовых оверлеев, кастомные кнопки-иконки, светлая/тёмная темы и собственный UI-шрифт — всё на базе собственного UI-тулкита проекта.
...
1. Запустите Improve-ImgSLI — откроется экран выбора сессии; выберите **Image Compare** для классического сравнения двух изображений или **Multi Compare**, чтобы расположить сразу несколько. Каждый выбор открывается в своей вкладке.
2. В Image Compare загрузите изображения через «Add Img(s)» или перетаскиванием. Для быстрого просмотра одного изображения используйте `Space + ЛКМ/ПКМ`.
...
7. В Multi Compare добавляйте изображения перетаскиванием или через тулбар, перетаскивайте слоты для перестройки layout'а, меняйте размеры разделителей и открывайте полноэкранный focus-режим двойным кликом по ячейке.
...
GPL-3.0 License. См. <a href="LICENSE">LICENSE</a>.
