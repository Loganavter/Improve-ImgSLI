...
- 🔍 Лупа: двойной или разделённый круг, интерполяции (Nearest/Bilinear/Bicubic/Lanczos), точное управление WASD/QE, фиксация позиции, «лазерные» направляющие.
- 🗂️ Потоки и файлы: drag-n-drop, переупорядочивание между списками, короткое/долгое нажатие, рейтинги, редактирование имён, вставка из буфера (Ctrl+V).
- 🎥 Запись и редактирование видео: запись сеансов сравнения, простая обрезка и экспорт в популярные форматы (MP4, WebM и др.).
- 🎨 Оверлеи и UI: полностью кастомный интерфейс с настраиваемыми разделителями и лупой (видимость, цвет, толщина), кастомными иконками, светлой/тёмной темами.
- 📤 Экспорт: WYSIWYG экспорт композита (сплиттер, лупа, текст), форматы PNG/JPEG/WEBP/BMP/TIFF/JXL, управление качеством, стили текста.
- ⚙️ UX и настройки: сохранение состояния окна/компоновки, мультиязычный интерфейс (EN/RU/zh/pt_BR), автообрезка чёрных полей, скрипт-лаунчер.
