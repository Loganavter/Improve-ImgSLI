...
- **For delegation (parallel `Task`): never work on `main`.** Supervisor must `git checkout -b feat/<slug>` or `git worktree add ../<repo>-worktree feat/<slug>` before spawning workers — the `reset --hard` fires only when `CUR_BRANCH=main` (`~/.local/bin/*-autopush.sh`), so on a branch the working tree keeps your edits. But the branch is **not** untouched: the backup commit stays in its history (it is what `backup/autopush` points at), so before opening a PR drop the `auto: periodic backup` commits (`git rebase -i main` / `git reset --soft main`). Pause with `systemctl --user stop *autopush.timer` if needed.
