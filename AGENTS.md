...
- **Working-tree edits get swept into backup commits mid-task.** Nothing is
  ...
  backup commit. If you need to know where your work ended up after a
  backup, run `git log --oneline -3 -- <file>` or `git log backup/autopush --oneline -5` rather than assuming HEAD.
- **Prefer `git worktree` over `git stash` for before/after comparisons.**
  `stash pop` still conflicts with tracked `__pycache__/*.pyc` (regenerated by pytest). Daemon now pushes to `backup/autopush` (not `main`, `reset --hard`), so `main` is stash-safe, but `worktree` is cleaner.
- **For delegation (parallel `Task`): never work on `main`.** Supervisor must `git checkout -b feat/<slug>` or `git worktree add ../<repo>-worktree feat/<slug>` before spawning workers — daemon only `reset`s `main` (`~/.local/bin/*-autopush.sh: CUR_BRANCH=main → reset`), branches/worktrees are untouched. Pause with `systemctl --user stop *autopush.timer` if needed.
