...
- **Working-tree edits get swept into backup commits mid-task.** Nothing is
  ...
  backup, run `git log --oneline -3 -- <file>` rather than assuming HEAD.
- **Never use `git stash` / `git stash pop` for before/after comparisons.**
  The daemon's mid-task commits plus the tracked `__pycache__/*.pyc` files
  (committed too, regenerated by every pytest run) turn a stash pop into a
  large conflict dump that is hard to unwind. If you need a clean baseline,
  use a separate `git worktree` instead.
