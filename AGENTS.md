...
`auto: periodic backup YYYY-MM-DD HH:MM [from $CUR_BRANCH]` and pushes to a **per-branch** ref `backup/autopush/<branch>` — `main` is PR-only. Same scheme in all three repos (`Improve-ImgSLI`, `improve-imgsli-internal-docs`, `sli-ui-toolkit`). The daemon never runs `git reset --hard` and never wipes the working tree. Consequences for agents:

- **Working-tree edits get swept into backup commits mid-task.** The script
  (`git add -A` + commit + `git branch -f backup/autopush/<branch> HEAD` + push)
  sweeps everything, including your index — staged-but-uncommitted work also
  lands in the `auto:` commit. Nothing is lost and nothing is wiped: recover with
  `git log backup/autopush/<branch> --oneline -5`
  (or `git checkout backup/autopush/<branch> -- <file>`).
- **Do not use `git stash` for before/after comparisons — it is denied by opencode permissions.** The stash list is shared per repo, so `stash pop` can apply someone else's stash. Use `git worktree` instead.
  Worktrees live in `/tmp/opencode/<repo>-<slug>` (never `../<repo>-worktree` — keeps project dirs clean; reboot wipes them, commits survive in the main `.git`). If `/tmp` is tmpfs-tight, use `~/.cache/opencode/`.
- **For delegation (parallel `Task`): never work on `main`, never share one checkout.** Supervisor must give each worker its own branch (`git checkout -b feat/<slug>` or separate clones per agent). The backup commit stays in that branch's history (it is what `backup/autopush/<branch>` points at), so before opening a PR drop the `auto: periodic backup` commits (`git rebase -i main` / `git reset --soft main`). Pause with `systemctl --user stop *autopush.timer` if needed. Note: worktrees share one `.git` (and its stash list) — separate clones isolate fully.
