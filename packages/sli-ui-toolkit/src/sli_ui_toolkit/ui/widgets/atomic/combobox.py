...
class _DropdownOverlay(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for visible_idx in range(visible_count):
            ...
            item_rect = QRectF(
                self._item_rect(visible_idx).translated(self._content_rect().topLeft()).adjusted(0, 1, -1, -1)
            )
...
class ComboBox(QWidget):
    ...
    def removeItem(self, index: int):
        ...
        self._scroll_offset = max(
            0,
            min(self._scroll_offset, max(0, self.count() - self._max_visible_items)),
        )
        self.update()
    ...
    def _draw_field(self, painter: QPainter):
        ...
        if current_text:
            ...
            painter.drawText(
                ...
                fm.elidedText(current_text, Qt.TextElideMode.ElideRight, text_rect.width()),
            )
