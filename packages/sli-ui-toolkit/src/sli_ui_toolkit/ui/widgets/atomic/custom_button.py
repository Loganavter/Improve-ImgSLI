...
class CustomButton(QWidget):
    ...
    def _resolve_icon(self, icon: Optional[Union[QIcon, Any]]) -> QIcon:
        if icon is None:
            return QIcon()
    ...
    def _on_theme_changed(self):
        ...
        text_color = self._foreground_color or self.theme_manager.get_color(text_color_key)
        palette = self.text_label.palette()
        palette.setColor(self.text_label.foregroundRole(), text_color)
    ...
    def setIcon(self, icon: Optional[Union[QIcon, Any]]):
        self._icon = icon
        self._rebuild_layout()
        self._update_icon_pixmap()
    ...
    def _style_prefix(self) -> str:
        return (
            "button.primary"
            if "primary" in str(self.property("class") or "")
            else "button.dialog.default"
        )
    ...
    def mouseReleaseEvent(self, e):
        self.setProperty("state", "hover" if self.rect().contains(e.pos()) else "normal")
        self.update()
        if (
            self.isEnabled()
    ...
    def paintEvent(self, _):
        ...
        if self._is_footer:
            ...
            path.arcTo(
                rect.right() - 2 * self.RADIUS,
                rect.bottom() - 2 * self.RADIUS,
                2 * self.RADIUS,
                2 * self.RADIUS,
                0,
                -90,
