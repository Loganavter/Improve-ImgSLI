...
try:
    ...
    logger.warning("'clickable_label.py' not found. Falling back to QLabel for image display.")
    ClickableLabel = QLabel


class LongPressButton(TransparentPushButton):
...
class ScrollableComboBox(ComboBox):
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        if delta > 0:
            new_index = (current_index - 1 + self.count()) % self.count()
        elif delta < 0:
...
class UILogic:
    ...
    def __init__(self, app_instance, app_state):
        ...
        self.app.btn_swap = LongPressButton()
        self.app.btn_clear_list1 = LongPressButton()
        self.app.btn_clear_list2 = LongPressButton()
        ...
        self.app.combo_image1 = ScrollableComboBox()
        self.app.combo_image2 = ScrollableComboBox()
    ...
    def build_all(self):
        self.app.setWindowTitle(
            tr('Improve ImgSLI', self.app_state.current_language))
        ...
        resolution_layout.addWidget(
            self.app.resolution_label1,
            alignment=Qt.AlignmentFlag.AlignLeft)
        resolution_layout.addStretch()
        resolution_layout.addWidget(
            self.app.resolution_label2,
    ...
    def _create_button_layout(self):
        ...
        self.app.btn_swap.setSizePolicy(
        ...
        self.app.btn_clear_list1.setSizePolicy(
        ...
        self.app.btn_clear_list2.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.app.btn_clear_list2.setStyleSheet(
            '\n            TransparentPushButton {\n                padding: 7px;\n                qproperty-iconSize: 22px;\n            }\n        ')
        self.app.btn_image1.setText(
            tr('Add Image(s) 1', self.app_state.current_language))
        self.app.btn_image2.setText(
            tr('Add Image(s) 2', self.app_state.current_language))
    ...
    def _create_combobox_layout(self):
        layout = QHBoxLayout()
        layout.setSpacing(8)
        self.app.combo_image1.setToolTip(
            tr('Select image for left/top side', self.app_state.current_language))
        self.app.combo_image2.setToolTip(
            tr('Select image for right/bottom side', self.app_state.current_language))
    ...
    def _create_checkbox_layout(self):
        ...
        self.app.checkbox_horizontal.setText(
            tr('Horizontal Split', self.app_state.current_language))
        self.app.checkbox_magnifier.setText(
            tr('Use Magnifier', self.app_state.current_language))
        self.app.freeze_button.setText(
            tr('Freeze Magnifier', self.app_state.current_language))
        self.app.checkbox_file_names.setText(
            tr('Include file names in saved image', self.app_state.current_language))
        ...
        self.app.btn_settings.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
    ...
    def _create_slider_layout(self):
        ...
        self.app.label_magnifier_size.setText(
            tr('Magnifier Size (%):', self.app_state.current_language))
        ...
        self.app.label_capture_size.setText(
            tr('Capture Size (%):', self.app_state.current_language))
        self.app.slider_capture.setMinimum(1)
        self.app.slider_capture.setMaximum(100)
        ...
        sliders_main_layout.addSpacing(15)
        self.app.label_movement_speed.setText(
            tr('Move Speed:', self.app_state.current_language))
        ...
        self.app.label_interpolation.setText(
            tr('Magnifier Interpolation:', self.app_state.current_language))
        self.app.combo_interpolation.blockSignals(True)
        self.app.combo_interpolation.clear()
        for i, (key_str, display_text_key) in enumerate(
                AppConstants.INTERPOLATION_METHODS_MAP.items()):
            if not hasattr(Image.Resampling, key_str):
                logger.warning(
                    f"PIL.Image.Resampling does not have method '{key_str}'. Skipping adding to ComboBox.")
                continue
            translated_display_text = tr(
                display_text_key, self.app_state.current_language)
            self.app.combo_interpolation.addItem(
                translated_display_text, userData=i)
            logger.debug(
                f"Added item '{translated_display_text}' with userData={i} for internal key '{key_str}'.")
    ...
    def _configure_image_label(self):
        ...
        self.app.image_label.setSizePolicy(
    ...
    def _create_file_names_layout(self):
        layout = QHBoxLayout()
        self.app.file_name_label1.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Preferred)
        self.app.file_name_label2.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Preferred)
        self.app.file_name_label1.setAlignment(
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
        self.app.file_name_label2.setAlignment(
        ...
        layout.addWidget(
            self.app.file_name_label1,
            alignment=Qt.AlignmentFlag.AlignLeft)
        layout.addStretch()
        layout.addWidget(
            self.app.file_name_label2,
            alignment=Qt.AlignmentFlag.AlignRight)
    ...
    def _create_edit_layout(self):
        ...
        self.app.label_edit_name1.setText(
            tr('Name 1:', self.app_state.current_language))
        self.app.edit_name1.setPlaceholderText(
            tr('Edit Current Image 1 Name', self.app_state.current_language))
        self.app.edit_name1.setMinimumHeight(28)
        self.app.label_edit_name2.setText(
            tr('Name 2:', self.app_state.current_language))
        self.app.edit_name2.setPlaceholderText(
            tr('Edit Current Image 2 Name', self.app_state.current_language))
        self.app.edit_name2.setMinimumHeight(28)
        self.app.label_edit_font_size.setText(
            tr('Font Size (%):', self.app_state.current_language))
        ...
        self.app.btn_color_picker.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
    ...
    def _create_save_buttons_widget(self):
        ...
        self.app.btn_save.setText(
            tr('Save Result', self.app_state.current_language))
    ...
    def _update_drag_overlays(self, horizontal: bool = False):
        if not hasattr(self.app, 'drag_overlay1') or not hasattr(
                self.app, 'image_label') or (not self.app.image_label.isVisible()):
            return
        try:
            ...
            if not horizontal:
                ...
                self.app.drag_overlay1.setGeometry(
                    margin, margin, overlay_w, overlay_h)
                self.app.drag_overlay2.setGeometry(
            ...
            logger.warning(
                f'Error updating drag overlays geometry in UILogic: {e}')

    def _init_warning_label(self):
        self.app.length_warning_label.setStyleSheet(
            'color: #FF4500; font-weight: bold;')
        self.app.length_warning_label.setAlignment(
    ...
    def _update_magnifier_controls_visibility(self, visible: bool):
        if hasattr(self.app, 'magnifier_settings_panel'):
            ...
            logger.debug(f'Magnifier panel visibility set to {visible}')
        else:
            logger.critical(
                'magnifier_settings_panel not found during visibility update!')
    ...
    def update_file_names(self):
        name1_from_edit = self.app.edit_name1.text().strip(
        ) if hasattr(self.app, 'edit_name1') else ''
        name2_from_edit = self.app.edit_name2.text().strip(
        ) if hasattr(self.app, 'edit_name2') else ''
        display_name1_for_label = name1_from_edit
        display_name2_for_label = name2_from_edit
        if not self.app_state.original_image1:
            display_name1_for_label = tr(
                'Image 1', self.app_state.current_language)
        if not self.app_state.original_image2:
            display_name2_for_label = tr(
                'Image 2', self.app_state.current_language)
        self.app_state.max_name_length
        if hasattr(self.app, 'file_name_label1') and hasattr(
                self.app, 'file_name_label2'):
            should_be_visible = self.app_state.original_image1 is not None and self.app_state.original_image2 is not None and (
            ...
            if should_be_visible:
                prefix1 = tr(
                    'Left',
                    self.app_state.current_language) if not self.app_state.is_horizontal else tr(
                    'Top',
                    self.app_state.current_language)
                prefix2 = tr(
                    'Right',
                ...
                self.app.file_name_label1.setText(
                    f'{prefix1}: {display_name1_for_label}')
                self.app.file_name_label2.setText(
                    f'{prefix2}: {display_name2_for_label}')
                self.app.file_name_label1.setToolTip(
                    display_name1_for_label if display_name1_for_label else tr(
                        'No name entered', self.app_state.current_language))
                self.app.file_name_label2.setToolTip(
                    display_name2_for_label if display_name2_for_label else tr(
                        'No name entered', self.app_state.current_language))
            else:
                self.app.file_name_label1.setText('')
                self.app.file_name_label2.setText('')
    ...
    def check_name_lengths(self, name1=None, name2=None):
        ...
        len1, len2, limit = (len(name1 or ''), len(
            name2 or ''), self.app_state.max_name_length)
        if (len1 > limit or len2 >
                limit) and self.app_state.include_file_names_in_saved:
            longest = max(len1, len2)
            warning_text = tr(
                'Name length limit ({limit}) exceeded!',
                self.app_state.current_language).format(
                limit=limit)
            tooltip_text = tr(
    ...
    def update_translations(self):
        ...
        if hasattr(self.app, 'combo_interpolation'):
            self.app.combo_interpolation.blockSignals(True)
            current_selected_user_data = self.app.combo_interpolation.itemData(
                self.app.combo_interpolation.currentIndex())
            self.app.combo_interpolation.clear()
            list(AppConstants.INTERPOLATION_METHODS_MAP.keys())
            for i, (key_str, display_text_key) in enumerate(
                    AppConstants.INTERPOLATION_METHODS_MAP.items()):
                if not hasattr(Image.Resampling, key_str):
                    continue
            ...
            if isinstance(current_selected_user_data, int):
                for i in range(self.app.combo_interpolation.count()):
                    if self.app.combo_interpolation.itemData(
                            i) == current_selected_user_data:
                        index_to_restore = i
                        break
        ...
        if hasattr(self.app, 'btn_image1'):
            ...
            self.app.btn_image1.setToolTip(
                tr('Add images to the left/top panel', lang))
        if hasattr(self.app, 'btn_image2'):
            self.app.btn_image2.setText(tr('Add Image(s) 2', lang))
            self.app.btn_image2.setToolTip(
                tr('Add images to the right/bottom panel', lang))
        if hasattr(self.app, 'btn_swap'):
            tooltip_text = f"{tr('Click: Swap current images',
                                 lang)}\n{tr('Hold: Swap entire lists',
                                             lang)}"
            self.app.btn_swap.setToolTip(tooltip_text)
        if hasattr(self.app, 'btn_clear_list1'):
            tooltip_text = f"{tr('Click: Remove current image', lang)}\n{tr('Hold: Clear entire list', lang)}"
            self.app.btn_clear_list1.setToolTip(tooltip_text)
        if hasattr(self.app, 'btn_clear_list2'):
            tooltip_text = f"{tr('Click: Remove current image', lang)}\n{tr('Hold: Clear entire list', lang)}"
            self.app.btn_clear_list2.setToolTip(tooltip_text)
        if hasattr(self.app, 'btn_save'):
            self.app.btn_save.setText(tr('Save Result', lang))
            self.app.btn_save.setToolTip(
                tr('Save Result outputs the current comparison view as an image file.', lang))
        ...
        if hasattr(self.app, 'btn_settings'):
            self.app.btn_settings.setToolTip(
                tr(
                    'Settings dialog module not found.',
                    lang) if not self.app.settings_dialog_available else tr(
        ...
        if hasattr(self.app, 'checkbox_file_names'):
            self.app.checkbox_file_names.setText(
                tr('Include file names in saved image', lang))
        if hasattr(self.app, 'label_magnifier_size'):
            self.app.label_magnifier_size.setText(
                tr('Magnifier Size (%):', lang))
        ...
        if hasattr(self.app, 'combo_image1'):
            self.app.combo_image1.setToolTip(
                tr('Select image for left/top side', lang))
        if hasattr(self.app, 'combo_image2'):
            self.app.combo_image2.setToolTip(
                tr('Select image for right/bottom side', lang))
        if hasattr(self.app, 'slider_speed'):
            self.app.slider_speed.setToolTip(
                f"{self.app_state.movement_speed_per_sec:.1f} {tr('rel. units/sec', lang)}")
        if hasattr(self.app, 'slider_size'):
            self.app.slider_size.setToolTip(
                f'{int(self.app_state.magnifier_size_relative * 100):.0f}%')
        if hasattr(self.app, 'slider_capture'):
            self.app.slider_capture.setToolTip(
                f'{int(self.app_state.capture_size_relative * 100):.0f}%')
        if hasattr(self.app, 'label_interpolation'):
            self.app.label_interpolation.setText(
                tr('Magnifier Interpolation:', lang))
        if hasattr(self.app, 'btn_color_picker'):
            self.app._update_color_button_tooltip()
        if hasattr(self.app,
                   'drag_overlay1') and self.app.drag_overlay1.isVisible():
            self.app.drag_overlay1.setText(tr('Drop Image(s) 1 Here', lang))
        if hasattr(self.app,
                   'drag_overlay2') and self.app.drag_overlay2.isVisible():
            self.app.drag_overlay2.setText(tr('Drop Image(s) 2 Here', lang))
        if hasattr(
                self.app, 'length_warning_label') and self.app.length_warning_label.isVisible():
