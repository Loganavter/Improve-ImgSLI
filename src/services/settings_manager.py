...
class SettingsManager:
        ...
        try:
            if target_type == int:
                ...
                if not isinstance(value, str):
                    print(f"[DEBUG] Warning: Expected Base64 string for QByteArray key '{key}', but got type {type(value)}. Using default.")
                    return QByteArray()
                if not value:
                    return QByteArray()
                try:
                    ...
                    if byte_data.isNull():
                        print(f"[DEBUG] Warning: Base64 decoding resulted in null QByteArray for key '{key}'. Value: '{value[:50]}...'")
                        return QByteArray()
                    return byte_data
                except Exception as e_b64:
                    print(f"[DEBUG] Warning: Error decoding Base64 string for QByteArray key '{key}'. Value: '{value[:50]}...', Error: {e_b64}")
                    traceback.print_exc()
                    return QByteArray()
            elif target_type == QPointF:
                if not isinstance(value, str):
                    print(f"[DEBUG] Warning: Expected string 'x,y' for QPointF key '{key}', but got type {type(value)}. Using default.")
                    return QPointF()
                try:
                    parts = value.split(',')
                    if len(parts) == 2:
                    ...
                    print(f"[DEBUG] Warning: Error parsing QPointF string for key '{key}'. Value: '{value}', Error: {e_point}")
                return QPointF()
                ...
                if isinstance(value, list):
                    if not value or all((isinstance(item, str) for item in value)):
                        ...
                        print(f"[DEBUG] Warning: List for key '{key}' contains non-string elements. Using default.")
                        return []
                else:
                    print(f"[DEBUG] Warning: Expected list for key '{key}', but got type {type(value)}. Using default.")
                    return []
            else:
                print(f"[DEBUG] Warning: Unhandled target type '{target_type.__name__}' for key '{key}'. Returning raw value.")
                return value
        except Exception as e_unexpected:
            print(f"[DEBUG] Unexpected error processing setting '{key}' for type {target_type.__name__}. Raw Value: '{value}', Error: {e_unexpected}")
            traceback.print_exc()
            return default

    def load_all_settings(self, app_state: AppState):
        app_state.loaded_geometry = self._get_setting('window_geometry', QByteArray(), QByteArray)
        app_state.loaded_was_maximized = self._get_setting('window_was_maximized', False, bool)
        app_state.loaded_previous_geometry = self._get_setting('previous_geometry', QByteArray(), QByteArray)
        ...
        app_state.max_name_length = max(AppConstants.MIN_NAME_LENGTH_LIMIT, min(AppConstants.MAX_NAME_LENGTH_LIMIT, self._get_setting('max_name_length', 30, int)))
        app_state.movement_speed_per_sec = max(0.1, min(5.0, self._get_setting('movement_speed_per_sec', 2.0, float)))
        default_color = QColor(255, 0, 0, 255)
        loaded_color_name = self._get_setting('filename_color', default_color.name(QColor.NameFormat.HexArgb), str)
        ...
        app_state.jpeg_quality = max(1, min(100, self._get_setting('jpeg_quality', AppConstants.DEFAULT_JPEG_QUALITY, int)))
        app_state.magnifier_offset_relative_visual = QPointF(app_state.magnifier_offset_relative)
    ...
    def save_all_settings(self, app_state: AppState, window_widget: QObject):
        runtime_prev_geom_valid = bool(app_state.loaded_previous_geometry is not None and isinstance(app_state.loaded_previous_geometry, QByteArray) and (not app_state.loaded_previous_geometry.isNull()) and (not app_state.loaded_previous_geometry.isEmpty()))
        ...
        if current_geometry and (not current_geometry.isEmpty()):
            ...
            try:
                ...
                print(f"[DEBUG] Error removing invalid 'window_geometry' on close: {e}")
        ...
        if should_save_as_maximized:
            if runtime_prev_geom_valid:
                self._save_setting('previous_geometry', app_state.loaded_previous_geometry)
            elif self.settings.contains('previous_geometry'):
                try:
                    self.settings.remove('previous_geometry')
                except Exception as e:
                    print(f"[DEBUG] Error removing 'previous_geometry' setting on close (maximized but no valid previous geom): {e}")
        elif self.settings.contains('previous_geometry'):
            try:
                self.settings.remove('previous_geometry')
            except Exception as e:
                print(f"[DEBUG] Error removing 'previous_geometry' setting on close (normal): {e}")
        ...
        self._save_setting('filename_color', app_state.file_name_color.name(QColor.NameFormat.HexArgb))
    ...
    def _save_setting(self, key: str, value):
        try:
            ...
            if isinstance(value, QPointF):
                ...
                print(f"[DEBUG] ERROR: Attempted to save unsupported type '{type(value)}' for key '{key}'. Skipping save.")
                return
            if value_to_save is not None:
                self.settings.setValue(key, value_to_save)
        except Exception as e:
            print(f"[DEBUG] ERROR saving setting '{key}' (value type: {type(value)}): {e}")
            traceback.print_exc()
