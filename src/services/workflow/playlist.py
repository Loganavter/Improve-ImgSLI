...
class PlaylistManager:
    ...
    def swap_current_images(self):
        ...
        self.store.viewport.image1, self.store.viewport.image2 = (
            self.store.viewport.image2,
            self.store.viewport.image1,
        )

        self.store.invalidate_geometry_cache()

        if self.main_controller:
            self.main_controller.ui_update_requested.emit(
                ["combobox", "file_names", "resolution"]
            )
    ...
    def move_item_between_lists(
        ...
        path1_before = (
            self.store.document.image_list1[self.store.document.current_index1].path
            if 0
            <= self.store.document.current_index1
        ...
        path2_before = (
            self.store.document.image_list2[self.store.document.current_index2].path
            if 0
            <= self.store.document.current_index2
        ...
        if path1_before:
            try:
                new_idx1 = next(
                    i
                    for i, item in enumerate(self.store.document.image_list1)
                    if item.path == path1_before
                )
        ...
        if new_idx1 == -1 and len(self.store.document.image_list1) > 0:
            ...
            if (
                source_list_num == 1
                and source_index == self.store.document.current_index1
            ):
        ...
        if path2_before:
            try:
                new_idx2 = next(
                    i
                    for i, item in enumerate(self.store.document.image_list2)
                    if item.path == path2_before
                )
            except StopIteration:
                pass
        if new_idx2 == -1 and len(self.store.document.image_list2) > 0:
            ...
            if (
                source_list_num == 2
                and source_index == self.store.document.current_index2
            ):
    ...
    def _call_set_current_image(self, image_number: int, emit_signal: bool = True):
        ...
        if hasattr(self.store, "set_current_image"):
            ...
            if self.main_controller and self.main_controller.session_ctrl:
                self.main_controller.session_ctrl.set_current_image(
    ...
    def _change_rating(self, image_number: int, index_to_change: int, delta: int):
        ...
        def _update_flyout():
            if (
                hasattr(self.main_controller, "presenter")
                and self.main_controller.presenter
            ):
