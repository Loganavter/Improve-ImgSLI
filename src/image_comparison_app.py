...
class ImageComparisonApp(QWidget):
    ...
    def _load_settings(self):
        ...
        valid_languages = ['en', 'ru', 'zh', 'pt_BR'] # Added pt_BR
        # Determine default language based on system locale
        default_lang = QLocale.system().name()[:2]
        if default_lang not in valid_languages: default_lang = 'en' # Default to 'en' if system locale is not supported
        ...
        # Initial empty lists for paths/indices - they are not saved/loaded in current design
    ...
    def _init_state(self):
        ...
        DEFAULT_MAGNIFIER_SIZE_RELATIVE = 0.20
        DEFAULT_CAPTURE_SIZE_RELATIVE = 0.10
        self.magnifier_size_relative = DEFAULT_MAGNIFIER_SIZE_RELATIVE
        self.capture_size_relative = DEFAULT_CAPTURE_SIZE_RELATIVE
        ...
        self.magnifier_offset_relative = QPointF(0.0, -0.5)
        self.magnifier_spacing_relative = 0.1
    ...
    def _update_magnifier_position_by_keys(self):
        ...
        # Предотвращение слишком больших скачков при лагах или возобновлении таймера
        if delta_time_ms <= 0 or delta_time_ms > 100:
            delta_time_ms = self.movement_timer.interval() # Используем интервал таймера как запасной вариант
        ...
        raw_dx_rel, raw_dy_rel, raw_ds_rel = 0.0, 0.0, 0.0
        epsilon = 1e-6 # Малое значение для сравнения float
        ...
        if self.active_keys:
            ...
            # Нормализуем вектор направления (для диагонального движения с той же скоростью)
            ...
            # Получаем текущий множитель скорости из слайдера (предполагаем, что он хранит значение от 0.1 до 5.0)
            # Если слайдер хранит 1-50, нужно скорректировать множитель, например:
            # speed_multiplier = self.movement_speed_per_sec / 10.0 # Пример: 1..5
            speed_multiplier = self.movement_speed_per_sec # Используем как есть, если он уже 0.1-5.0

            # --- Логика движения ---
            target_movement_applied = False
            if dx_dir != 0 or dy_dir != 0:
                if self.freeze_magnifier:
                    # --- Движение замороженной лупы (ПИКСЕЛЬНАЯ СКОРОСТЬ) ---
                    if self.frozen_magnifier_position_relative and self.pixmap_width > 0 and self.pixmap_height > 0:
                        ...
                        # Рассчитываем целевое смещение в пикселях экрана
                        ...
                        frozen_pix_x_current = self.frozen_magnifier_position_relative.x() * self.pixmap_width
                        frozen_pix_y_current = self.frozen_magnifier_position_relative.y() * self.pixmap_height

                        # Новая целевая позиция в пикселях
                        new_frozen_pix_x = frozen_pix_x_current + target_pixel_dx
                        new_frozen_pix_y = frozen_pix_y_current + target_pixel_dy

                        # Преобразуем обратно в относительные координаты и ограничиваем 0.0-1.0
                        ...
                        # Обновляем, если позиция изменилась
                        if not math.isclose(new_x_rel, self.frozen_magnifier_position_relative.x(), abs_tol=epsilon) or \
                        not math.isclose(new_y_rel, self.frozen_magnifier_position_relative.y(), abs_tol=epsilon):
                            ...
                            target_movement_applied = True

                else:
                    # --- Движение незамороженной лупы (ОТНОСИТЕЛЬНАЯ СКОРОСТЬ СМЕЩЕНИЯ) ---
                    relative_speed = BASE_RELATIVE_SPEED_UNFROZEN * speed_multiplier
                    # Рассчитываем изменение относительного смещения
                    delta_offset_x = dx_dir * relative_speed * delta_time_sec
                    delta_offset_y = dy_dir * relative_speed * delta_time_sec

                    # Ограничиваем максимальный шаг за один тик (опционально, но может помочь)
                    ...
                    # Применяем изменение к ЦЕЛЕВОМУ смещению
                    new_target_x = self.magnifier_offset_relative.x() + clamped_dx_rel
                    new_target_y = self.magnifier_offset_relative.y() + clamped_dy_rel

                    # Обновляем, если смещение изменилось (не обязательно ограничивать, т.к. визуально оно интерполируется)
                    if not math.isclose(new_target_x, self.magnifier_offset_relative.x(), abs_tol=epsilon) or \
                    not math.isclose(new_target_y, self.magnifier_offset_relative.y(), abs_tol=epsilon):
                        self.magnifier_offset_relative.setX(new_target_x)
                        self.magnifier_offset_relative.setY(new_target_y)
                        target_pos_changed = True # Сигнализируем, что цель изменилась
                        target_movement_applied = True

            # --- Логика изменения расстояния (Q/E) ---
            if ds_dir != 0:
                spacing_speed = self.spacing_speed_per_sec_qe # Используем отдельную скорость для этого
                delta_spacing = ds_dir * spacing_speed * delta_time_sec
                clamped_ds_rel = max(-self.max_target_delta_per_tick, min(self.max_target_delta_per_tick, delta_spacing)) # Ограничение шага

                new_target_spacing = max(0.0, min(0.5, self.magnifier_spacing_relative + clamped_ds_rel))
                if not math.isclose(new_target_spacing, self.magnifier_spacing_relative, abs_tol=epsilon):
                    self.magnifier_spacing_relative = new_target_spacing
                    target_spacing_changed = True

        # --- Интерполяция визуальных параметров к целевым ---
        ...
        if not self.freeze_magnifier:
            ...
            # Если близко к цели, привязываемся точно
            if abs(delta_vx) < self.lerp_stop_threshold and abs(delta_vy) < self.lerp_stop_threshold:
                # Условие нужно, чтобы не вызывать перерисовку постоянно, если уже достигли цели
                if not math.isclose(self.magnifier_offset_relative_visual.x(), self.magnifier_offset_relative.x(), abs_tol=epsilon) or \
                not math.isclose(self.magnifier_offset_relative_visual.y(), self.magnifier_offset_relative.y(), abs_tol=epsilon):
        ...
        if abs(delta_vs) < self.lerp_stop_threshold:
            ...
            # Убедимся, что визуальный spacing не станет отрицательным
            self.magnifier_spacing_relative_visual = max(0.0, new_visual_spacing)
            visual_spacing_moved = True

        # --- Перерисовка и остановка таймера ---
        ...
        if needs_redraw and not self.resize_in_progress:
            self.update_comparison() # Вызываем перерисовку

        # Проверяем, нужно ли остановить таймер (если клавиши не нажаты И визуальные параметры достигли цели)
        if not self.active_keys:
            pos_is_close = self.freeze_magnifier or \
                        (abs(delta_vx) < self.lerp_stop_threshold and abs(delta_vy) < self.lerp_stop_threshold)
            spacing_is_close = abs(delta_vs) < self.lerp_stop_threshold

            if pos_is_close and spacing_is_close:
                self.movement_timer.stop()
                # Финальная установка точных значений на случай, если остановились чуть не доходя
                ...
                if needs_final_set and not self.resize_in_progress:
                    self.update_comparison() # Последняя перерисовка с точными значениями
    ...
    def _unfreeze_magnifier_logic(self):
        ...
        if frozen_pos_rel and self.pixmap_width > 0 and self.pixmap_height > 0:
             try:
                  target_min_dim = min(self.pixmap_width, self.pixmap_height); magnifier_size_pixels = max(1, int(self.magnifier_size_relative * target_min_dim))
                  ...
                  REFERENCE_MAGNIFIER_RELATIVE_SIZE = 0.2; reference_magnifier_size_display = max(1.0, REFERENCE_MAGNIFIER_RELATIVE_SIZE * float(target_min_dim))
    ...
    def change_language(self, language):
        ...
        if language not in valid_languages:
            print(f"Warning: Invalid language '{language}' requested. Defaulting to 'en'.")
            language = 'en'
    ...
    def update_minimum_window_size(self):
        ...
        try:
            ...
            img_label_hint = self.image_label.sizeHint()
            base_min_w = 200
            base_min_h = 250
    ...
    def _update_drag_overlays(self):
        ...
        try:
             ...
             overlay_w = max(1, half_width - margin - (margin // 2)); overlay_h = max(1, label_geom.height() - 2 * margin)
