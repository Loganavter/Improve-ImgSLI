...
class ImageComparisonApp(QWidget):
    ...
    def dropEvent(self, event):
            ...
            if url.isLocalFile():
                ...
                try:
                    ...
                    if ext in ('.png', '.jpg', '.jpeg', '.bmp', '.webp', '.tif', '.tiff'):
                        ...
                        unsupported_files.append(display_name + " (Unsupported ext)")
                except Exception as e:
        ...
        if not local_file_paths:
             print("No valid local image file paths found to process.")
             reason_list = []
             ...
             return
        print(f"Attempting to load {len(local_file_paths)} local file paths for slot {target_image_num}...")
    ...
    def _load_images_from_paths(self, file_paths: list[str], image_number: int):
        ...
        for file_path in file_paths:
            if not isinstance(file_path, str) or not file_path:
                print(f"  Skipping invalid item: {file_path}")
        ...
        if loaded_count > 0:
            ...
            if newly_added_indices:
             ...
             print(f"  Warning: Processed {len(file_paths)} paths, but loaded 0 new images for slot {image_number}.")
        if load_errors:
        ...
        if new_freeze_state:
            if self.use_magnifier and self.original_image1 and self.original_image2:
                ...
                if result_width > 0 and result_height > 0:
                    ...
                    if magnifier_pos_result_current and cap_orig1 is not None:
    ...
    def _edit_length_dialog(self, event):
        ...
        new_limit, ok = QInputDialog.getInt(
            ...
            tr("Enter new maximum length for UI display (10-100):", self.current_language),
