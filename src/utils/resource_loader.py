...
def get_magnifier_drawing_coords(
    ...
    thickness_float = CAPTURE_THICKNESS_FACTOR * math.sqrt(max(1.0, float(capture_size_on_unified)))
    thickness_val = max(float(MIN_CAPTURE_THICKNESS), min(float(MAX_CAPTURE_THICKNESS), thickness_float))
    thickness = max(2, int(round(thickness_val)))

    inner_capture_size_on_unified = max(1, capture_size_on_unified - thickness * 2)
    if inner_capture_size_on_unified % 2 != 0:
        inner_capture_size_on_unified += 1

    safe_radius = (capture_size_on_unified / 2.0) + (thickness_val / 2.0)

    radius_rel_x = safe_radius / unified_width if unified_width > 0 else 0.0
    radius_rel_y = safe_radius / unified_height if unified_height > 0 else 0.0
