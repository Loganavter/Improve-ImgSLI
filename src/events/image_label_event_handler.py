...
class ImageLabelEventHandler(QObject):
    ...
    def handle_mouse_move(self, event: QMouseEvent):
        ...
        user_fps = 0
        if hasattr(self.store, 'settings'):
        ...
        if user_fps > 0:
            ...
            target_fps = int(round(screen.refreshRate())) if screen else 60
            if target_fps < 30: target_fps = 60

        interval_ms = int(1000 / (target_fps * 1.2))

        if self._mouse_move_timer.elapsed() < interval_ms:
            return
