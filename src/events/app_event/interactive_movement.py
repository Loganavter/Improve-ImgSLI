...
from .interactive_movement_input import (
    apply_offset_input,
    apply_spacing_input,
...
from .interactive_movement_math import damp, damp_vector, is_close
...
class InteractiveMovementController:
    ...
    def handle_timer_tick(self) -> None:
        ...
        self._set_interactive_mode(True)
        if state_changed or self.store.viewport.view_state.optimize_interactive_movement:
            emit_update_request(self.presenter)
    ...
    def _apply_keyboard_input(self, delta_time_sec: float, handler) -> bool:
        ...
        if offset_changed:
            ...
            self._snap_visual_offset_on_direction_change(previous_dirs, self._last_input_dirs, handler)
            self.store.emit_viewport_change("interaction")
    ...
    def _should_finish_cycle(self, delta_time_sec: float, handler) -> bool:
        ...
        movement_keys_pressed = any(key in self.OVERLAY_MOVEMENT_KEYS for key in pressed_keys_set)
        is_user_inputting = (
        ...
        is_converged = (
            ...
            and abs(new_spacing_visual - (target_spacing or 0.0))
            < stop_threshold
    ...
    def _snap_visual_offset_on_direction_change(self, previous_dirs, current_dirs, handler) -> None:
        ...
        if snapped_x != visual.x or snapped_y != visual.y:
            self._set_visual_state(
                offset=Point(snapped_x, snapped_y)
            )
