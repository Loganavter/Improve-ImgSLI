...
def create_nine_slice_template(radius: int, fill: tuple, scale: int = 4) -> Image.Image:
    ...
    template = hr_canvas.resize(
        (radius * 2 + 1, radius * 2 + 1), Image.Resampling.LANCZOS
    )
    _nine_slice_template_cache[cache_key] = template
    return template

def draw_nine_slice_rounded_rect(
    target_image: Image.Image, rect: list, radius: int, fill: tuple
):
    ...
    if width < radius * 2 or height < radius * 2:
        ...
        if safe_radius > 0:
            ...
            ImageDraw.Draw(tmp).rounded_rectangle(
                (0, 0, width, height), radius=safe_radius, fill=fill
            )
    ...
    target_image.alpha_composite(
        template.crop((ts - c, ts - c, ts, ts)), (right - c, bottom - c)
    )

    if width > 2 * c:
        target_image.alpha_composite(
            template.crop((c, 0, ts - c, c)).resize((width - 2 * c, c)), (left + c, top)
        )
        target_image.alpha_composite(
            template.crop((c, ts - c, ts - c, ts)).resize((width - 2 * c, c)),
            (left + c, bottom - c),
        )
    if height > 2 * c:
        target_image.alpha_composite(
            template.crop((0, c, c, ts - c)).resize((c, height - 2 * c)),
            (left, top + c),
        )
        target_image.alpha_composite(
            template.crop((ts - c, c, ts, ts - c)).resize((c, height - 2 * c)),
            (right - c, top + c),
        )

    if width > 2 * c and height > 2 * c:
        target_image.alpha_composite(
            template.crop((c, c, ts - c, ts - c)).resize(
...
class TextDrawer:
    ...
    def _get_cached_text_size(
        self, text: str, font: FontType, font_weight: int
    ) -> Tuple[int, int]:
    ...
    def _draw_text_with_supersampling_stroke(
        self,
        ...
        base_color = (
            *fill_color[:3],
            int((fill_color[3] if len(fill_color) > 3 else 255) * alpha_percent / 100),
        )
        ...
        d.text(
            ...
            stroke_fill=base_color,
        )

        final_w = hr_w // scale
        final_h = hr_h // scale
        if final_w > 0 and final_h > 0:
            final_text_img = txt_canvas.resize(
    ...
    def draw_filenames_on_image(
        ...
        font = layout["font"]
        t_color = layout["text_rgba"]
        f_weight = layout["font_weight"]
        alpha_pc = layout["alpha_percent"]
    ...
    def compute_filenames_layout(
        ...
        txt1 = truncate_text(
            name1,
            int(w1_avail - PADDING_X * 2),
            name_limit,
            font,
            lambda t, f: self._get_cached_text_size(t, f, f_weight),
        )
        txt2 = truncate_text(
            name2,
            int(w2_avail - PADDING_X * 2),
            name_limit,
            font,
            lambda t, f: self._get_cached_text_size(t, f, f_weight),
        )
        ...
        def add_label(display_text, align, slot_num):
            if not display_text:
                return
            ...
            bg_qrect = QRect(
                int(bg_rect[0]),
                int(bg_rect[1]),
                int(bg_rect[2] - bg_rect[0]),
                int(bg_rect[3] - bg_rect[1]),
            )
            if visible_rect is not None and not bg_qrect.intersects(visible_rect):
                return
            ...
            labels.append(
                {
                    "text": display_text,
                    "bg_rect": bg_rect,
                    "text_x": int(bg_rect[0] + PADDING_X),
                    "text_y": int(bg_rect[1] + (bh - actual_th) // 2),
                    "text_w": int(tw),
                    "text_h": int(actual_th),
                }
            )
        ...
        if not store.viewport.is_horizontal:
            ...
            add_label(txt2, mode2, 2)
        else:
            add_label(txt1, "center", 1)
            add_label(txt2, "center", 2)

        return {
            "font": font,
            "font_size": int(font_size),
            "font_weight": int(f_weight),
            "alpha_percent": int(alpha_pc),
            "text_rgba": t_color,
            "bg_rgba": eff_bg_color,
            "labels": labels,
        }
