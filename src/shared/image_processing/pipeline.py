...
def _clamp_capture_position(rel_pos_x: float, rel_pos_y: float, canvas_width: int, canvas_height: int, capture_size_relative: float) -> Tuple[float, float]:
    ...
    thickness_factor = AppConstants.CAPTURE_THICKNESS_FACTOR
    thickness_float = thickness_factor * math.sqrt(max(1.0, float(capture_size_on_unified)))
    thickness = max(float(AppConstants.MIN_CAPTURE_THICKNESS), min(float(AppConstants.MAX_CAPTURE_THICKNESS), thickness_float))

    safe_radius_x = (capture_size_on_unified / 2.0) + (thickness / 2.0)
    safe_radius_y = (capture_size_on_unified / 2.0) + (thickness / 2.0)

    radius_rel_x = safe_radius_x / canvas_width if canvas_width > 0 else 0.0
    radius_rel_y = safe_radius_y / canvas_height if canvas_height > 0 else 0.0
...
class RenderingPipeline:
    ...
    def render_frame(self, ctx: RenderContext) -> dict:
        ...
        if not img1_scaled and not img2_scaled:
            ...
            return {"final_canvas": placeholder, "padding_left": 0, "padding_top": 0, "is_interactive": False}
        ...
        try:
            ...
            if ctx.is_export and ctx.use_magnifier and ctx.magnifier_drawing_coords:
            ...
            current_bg_key = (
                id(img1_scaled), id(img2_scaled), ctx.split_pos, ctx.is_horizontal,
                ctx.diff_mode, ctx.channel_view_mode
            )

            if ctx.background_cache_dict is not None and not ctx.is_export:
            ...
            if base_image is None:
                ...
                if not ctx.is_export and ctx.background_cache_dict is not None and base_image:
            ...
            if ctx.is_export:
                ...
                if ctx.use_magnifier and ctx.magnifier_drawing_coords:
                    ...
                    if ctx.show_magnifier_guides and mag_mid_on_canvas:
                        self._draw_magnifier_guides(ctx, final_canvas, mag_mid_on_canvas,
                                                   padding_left, padding_top, img_w, img_h)

                if ctx.use_magnifier and ctx.show_capture_area:

                     self._draw_capture_area_on_pil(ctx, final_canvas, padding_left, padding_top, img_w, img_h)

                if ctx.use_magnifier and ctx.magnifier_drawing_coords:
                     combined_center = self._draw_magnifier_full(
                        ctx, final_canvas, img1_scaled, img2_scaled,
                        padding_left, padding_top, img_w, img_h
                    )
            ...
            if ctx.use_magnifier and ctx.magnifier_drawing_coords:
                mag_bbox = ctx.magnifier_drawing_coords[5] if len(ctx.magnifier_drawing_coords) > 5 else None
                if not mag_bbox or not isinstance(mag_bbox, QRect) or mag_bbox.isEmpty():
                     s = ctx.magnifier_drawing_coords[3]
                     mag_bbox = QRect(0, 0, s, s)

                margin = 2
                overlay_w = mag_bbox.width() + margin * 2
                overlay_h = mag_bbox.height() + margin * 2

                overlay_image = Image.new("RGBA", (overlay_w, overlay_h), (0,0,0,0))

                global_center = ctx.magnifier_drawing_coords[2]
                local_center_x = global_center.x() - (mag_bbox.left() - margin)
                local_center_y = global_center.y() - (mag_bbox.top() - margin)
                local_center = QPoint(int(local_center_x), int(local_center_y))

                self._draw_magnifier_only(ctx, overlay_image, local_center, img1_scaled, img2_scaled)

                overlay_pos = QPoint(mag_bbox.left() - margin, mag_bbox.top() - margin)

            ui_data = self._calculate_ui_overlays(ctx, 0, 0, img_w, img_h,
                                                ctx.magnifier_drawing_coords[2] if ctx.magnifier_drawing_coords else QPoint(0,0))
    ...
    def _draw_magnifier_only(self, ctx: RenderContext, target_image: Image.Image,
                             display_center_pos: QPoint, img1: Image.Image, img2: Image.Image) -> Optional[QPoint]:
        """Рисует только лупу на маленьком прозрачном холсте (для интерактивного режима). Возвращает combined_center_point."""
        if not ctx.magnifier_drawing_coords:
            return None
        ...
        if ctx.magnifier_drawing_coords[2] is not None:
            ...
            mag_mid_on_canvas = display_center_pos

            original_img1 = ctx.original_image1 or img1
            original_img2 = ctx.original_image2 or img2
            ...
            interp = ctx.magnifier_movement_interpolation_method if ctx.is_interactive_mode else ctx.interpolation_method

            interactive_center_local = self.magnifier_drawer.draw_magnifier(
                temp_store, target_image,
                original_img1, original_img2,
                ctx.magnifier_drawing_coords[0], ctx.magnifier_drawing_coords[1],
                mag_mid_on_canvas,
                ctx.magnifier_drawing_coords[3], ctx.magnifier_drawing_coords[4],
                interp, ctx.is_horizontal, ctx.is_magnifier_combined,
                is_interactive_render=ctx.is_interactive_mode,
                internal_split=ctx.magnifier_internal_split,
                divider_visible=ctx.magnifier_divider_visible,
                divider_color=ctx.magnifier_divider_color,
            ...
            if interactive_center_local:
                combined_center_point = interactive_center_local
    ...
    def _draw_magnifier_guides(self, ctx: RenderContext, canvas: Image.Image,
        ...
        # Увеличиваем радиусы на половину толщины линии + небольшой зазор (0.5 - 1.0 пикселя)
        laser_gap = (ctx.magnifier_guides_thickness / 2.0) + 0.5
        
        r_mag = (mag_size / 2.0) + laser_gap

        raw_cap_size = max(5, int(round(ctx.capture_size * ref_dim)))
        r_cap = (raw_cap_size / 2.0) + laser_gap
    ...
    def _calculate_ui_overlays(self, ctx: RenderContext, padding_left: int, padding_top: int, img_w: int, img_h: int, mag_center: QPoint) -> dict:
        ui_data = {
            "capture_rect": None, "capture_color": None, "lasers": [],
            "divider_visible": False, "divider_pos": 0, "divider_horizontal": False,
            "divider_color": None, "divider_thickness": 0, "img_rect": None
        }

        from shared.image_processing.pipeline import _clamp_capture_position
        from PyQt6.QtGui import QColor

        offset = QPoint(padding_left, padding_top)

        ui_data["img_rect"] = QRect(padding_left, padding_top, img_w, img_h)

        if ctx.divider_line_visible and ctx.divider_line_thickness > 0 and ctx.diff_mode == 'off':
            ui_data["divider_visible"] = True
            ui_data["divider_thickness"] = ctx.divider_line_thickness
            c = ctx.divider_line_color
            ui_data["divider_color"] = QColor(c[0], c[1], c[2], c[3])
            ui_data["divider_horizontal"] = ctx.is_horizontal

            if not ctx.is_horizontal:

                ui_data["divider_pos"] = int(padding_left + round(img_w * ctx.split_pos))
            else:

                ui_data["divider_pos"] = int(padding_top + round(img_h * ctx.split_pos))
        else:
        ...
        capture_center_point = QPoint(0, 0)
        if ctx.use_magnifier and ctx.show_capture_area:
            ref_dim = min(img_w, img_h)
            ...
            cap_x_float = clamped_x * img_w + offset.x()
            cap_y_float = clamped_y * img_h + offset.y()

            top_left_x = round(cap_x_float - cap_size / 2.0)
            top_left_y = round(cap_y_float - cap_size / 2.0)

            ui_data["capture_rect"] = QRect(
                int(top_left_x),
                int(top_left_y),
                cap_size,
                cap_size
            )
            capture_center_point = QPoint(int(round(cap_x_float)), int(round(cap_y_float)))

            c = ctx.capture_ring_color
            ui_data["capture_color"] = QColor(c[0], c[1], c[2], c[3])

        if ctx.use_magnifier and ctx.show_magnifier_guides and ctx.magnifier_drawing_coords:

            mag_center_widget = mag_center + offset

            mag_size = ctx.magnifier_drawing_coords[3]
            spacing = ctx.magnifier_drawing_coords[4]

            laser_starts = self._get_laser_start_points(ctx, mag_center_widget, mag_size, spacing)

            l_c = ctx.magnifier_laser_color
            l_color = QColor(l_c[0], l_c[1], l_c[2], l_c[3])
            width = ctx.magnifier_guides_thickness

            # Вводим зазор для UI отрисовки
            ui_laser_gap = (width / 2.0) + 1.0 
            r_mag = (mag_size / 2.0) + ui_laser_gap

            ref_dim = min(img_w, img_h)
            raw_cap_size = max(5, int(round(ctx.capture_size * ref_dim)))
            r_cap = (raw_cap_size / 2.0) + ui_laser_gap

            for start_p in laser_starts:
                p1, p2 = self._clip_line_by_circles(start_p, capture_center_point, r_mag, r_cap)
                if p1 and p2:
                    ui_data["lasers"].append((p1, p2, l_color, width))

        if ctx.include_file_names:

            fc = ctx.file_name_color
            bg = ctx.file_name_bg_color

            ui_data["text_data"] = {
                "visible": True,
                "text1": ctx.file_name1,
                "text2": ctx.file_name2,
                "color": QColor(fc[0], fc[1], fc[2], fc[3]),
                "bg_color": QColor(bg[0], bg[1], bg[2], int(bg[3] * (ctx.text_alpha_percent / 100.0))),
                "font_size_percent": ctx.font_size_percent,
                "font_weight": ctx.font_weight,
                "draw_bg": ctx.draw_text_background,
                "placement": ctx.text_placement_mode,
                "is_horizontal": ctx.is_horizontal,
                "max_name_length": ctx.max_name_length
            }
        else:
            ui_data["text_data"] = {"visible": False}

        return ui_data

    def _draw_capture_area_on_pil(self, ctx: RenderContext, canvas: Image.Image, padding_left: int, padding_top: int, img_w: int, img_h: int):
        from shared.image_processing.pipeline import _clamp_capture_position
        from shared.image_processing.drawing.magnifier_drawer import CAPTURE_THICKNESS_FACTOR, MIN_CAPTURE_THICKNESS, MAX_CAPTURE_THICKNESS

        ref_dim = min(img_w, img_h)
        capture_size_px = max(1, int(round(ctx.capture_size * ref_dim)))

        thickness_float = CAPTURE_THICKNESS_FACTOR * math.sqrt(max(1.0, float(capture_size_px)))
        thickness_clamped = max(float(MIN_CAPTURE_THICKNESS), min(float(MAX_CAPTURE_THICKNESS), thickness_float))
        explicit_thickness = max(2, int(round(thickness_clamped)))
        ...
        cap_x_float = padding_left + clamped_x * img_w
        cap_y_float = padding_top + clamped_y * img_h
        center_qpointf = QPointF(cap_x_float, cap_y_float)
        ...
        self.magnifier_drawer.draw_capture_area(
            canvas, center_qpointf, capture_size_px,
            thickness=explicit_thickness,
            color=capture_color
        )

    def _get_laser_start_points(self, ctx: RenderContext, center: QPoint, size: int, spacing: int) -> list:
        points = []
        is_visual_diff = ctx.diff_mode in ('highlight', 'grayscale', 'ssim', 'edges')
        ...
        if ctx.is_magnifier_combined:
            if is_visual_diff:
                ...
                mid_x, mid_y = center.x(), center.y()
                if not ctx.is_horizontal:
                    combined_y = int(round(mid_y + size + 8))
                    points.append(QPoint(mid_x, combined_y))
                else:
                    combined_x = int(round(mid_x + size + 8))
                    points.append(QPoint(combined_x, mid_y))
            else:
                points.append(center)
        else:
            if is_visual_diff:
                spacing_f = float(spacing)
                offset = max(size, size + spacing_f)
                if not ctx.is_horizontal:
                    if ctx.magnifier_visible_left:
                        points.append(QPoint(int(round(center.x() - offset)), int(round(center.y()))))
                    if ctx.magnifier_visible_center:
                        points.append(center)
                    if ctx.magnifier_visible_right:
                        points.append(QPoint(int(round(center.x() + offset)), int(round(center.y()))))
                    ...
                    if ctx.magnifier_visible_left:
                        points.append(QPoint(int(round(center.x())), int(round(center.y() - offset))))
                    if ctx.magnifier_visible_center:
                        points.append(center)
                    if ctx.magnifier_visible_right:
                        points.append(QPoint(int(round(center.x())), int(round(center.y() + offset))))
                ...
                if not ctx.magnifier_is_horizontal:
                    if ctx.magnifier_visible_left:
                        points.append(QPoint(int(center.x() - offset), center.y()))
                    if ctx.magnifier_visible_right:
                        points.append(QPoint(int(center.x() + offset), center.y()))
                else:
                    if ctx.magnifier_visible_left:
                        points.append(QPoint(center.x(), int(center.y() - offset)))
                    if ctx.magnifier_visible_right:
                        points.append(QPoint(center.x(), int(center.y() + offset)))
        return points

    def _clip_line_by_circles(self, p1: QPoint, p2: QPoint, r1: float, r2: float) -> Tuple[Optional[QPoint], Optional[QPoint]]:
        import math
        dx = p2.x() - p1.x()
        dy = p2.y() - p1.y()
        dist = math.hypot(dx, dy)
        ...
        start = QPoint(int(round(p1.x() + nx * r1)), int(round(p1.y() + ny * r1)))
        end = QPoint(int(round(p2.x() - nx * r2)), int(round(p2.y() - ny * r2)))
