...
from toolkit.widgets.composite.unified_flyout.panel import _ListOwnerProxy, _Panel
...
class UnifiedFlyout(QWidget):
    ...
    def _calc_panel_total_size(self, list_num: int) -> QSize:
        ...
        font = panel.item_font if panel.item_font else QApplication.font()
        ...
        for i in range(layout.count()):
            ...
            if item and item.widget():
                ...
                if hasattr(widget, 'name_label'):
                    text = widget.name_label.text()
                    text_w = fm.horizontalAdvance(text)

                    total_w = text_w + 85
                    if total_w > max_item_width:
                        max_item_width = total_w
        ...
        logger.debug(f"[UnifiedFlyout] _calc_panel_total_size(list_num={list_num}, mode={self.mode}): button_width={button_width}, max_item_width={max_item_width}, final=({w}, {h})")
    ...
    def _update_geometry_in_double_mode_internal(self):
        ...
        button1.updateState(count1, idx1, text=text1, items=items1, force_sync_resize=True)
        button2.updateState(count2, idx2, text=text2, items=items2, force_sync_resize=True)
