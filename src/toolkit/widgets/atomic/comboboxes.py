...
from toolkit.widgets.helpers.underline_painter import (
    UnderlineConfig,
...
class ScrollableComboBox(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._flyout_closed_timestamp = 0.0
    ...
    def updateState(self, count: int, current_index: int, text: str = "", items: list = None, force_sync_resize: bool = False):
        self._count = count
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if self._pressed:
                ...
                if (time.time() - self._flyout_closed_timestamp) < 0.2:
                    self._pressed = False
                    self.update()
                    event.accept()
                    logger.debug("ScrollableComboBox: Click ignored due to debounce (closed recently)")
                    return
    ...
    def setFlyoutOpen(self, is_open: bool):
        if is_open:
            ...
            self._flyout_closed_timestamp = time.time()
