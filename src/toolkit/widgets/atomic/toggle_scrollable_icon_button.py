...
class ToggleScrollableIconButton(QWidget):
    ...
    def paintEvent(self, event):
        ...
        effective_underline_color = self._underline_color
        if self._current_value == 0:
            effective_underline_color = None

        if self._show_underline and effective_underline_color is not None:
            config = UnderlineConfig(
                thickness=1.0,
                vertical_offset=1.0,
                arc_radius=2.0,
                alpha=effective_underline_color.alpha() if effective_underline_color.alpha() < 255 else 200,
                color=effective_underline_color
            )
