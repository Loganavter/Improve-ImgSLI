...
class ThemeManager(QObject):
    ...
    def apply_theme_to_widget(self, widget):
        """Применяет тему к виджету, используя QSS с заменой плейсхолдеров"""
        palette_data = self._dark_palette if self.is_dark() else self._light_palette

        if not palette_data:
            theme_logger.warning("No palettes registered, skipping widget theme application")
            return

        # Применяем палитру
        q_palette = widget.palette()
        color_roles = {
            "Window": QPalette.ColorRole.Window,
            "WindowText": QPalette.ColorRole.WindowText,
            "Base": QPalette.ColorRole.Base,
        ...
        widget.setPalette(q_palette)

        # Применяем QSS с заменой плейсхолдеров
        if self._qss_template:
            ...
            widget.setStyleSheet(current_qss)
