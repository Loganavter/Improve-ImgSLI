...
def get_pil_resampling_method(method_name: str, is_interactive_render: bool):
    method_map_base = {'NEAREST': Image.Resampling.NEAREST, 'BILINEAR': Image.Resampling.BILINEAR, 'BICUBIC': Image.Resampling.BICUBIC, 'LANCZOS': Image.Resampling.LANCZOS}
    ...
    selected_pil_method = method_map_base.get(method_name.upper(), Image.Resampling.LANCZOS)
...
def resize_images_processor(original_image1: Image.Image | None, original_image2: Image.Image | None, interpolation_method: str) -> Tuple[Image.Image | None, Image.Image | None]:
    ...
    resampling_method_for_mode = get_pil_resampling_method(interpolation_method, False)
    final_processed_image1 = None
    final_processed_image2 = None
    try:
        if processed_img1_intermediate:
            if processed_img1_intermediate.size != target_size_final:
                final_processed_image1 = processed_img1_intermediate.resize(target_size_final, resampling_method_for_mode).copy()
            else:
                final_processed_image1 = processed_img1_intermediate
        if processed_img2_intermediate:
            if processed_img2_intermediate.size != target_size_final:
                final_processed_image2 = processed_img2_intermediate.resize(target_size_final, resampling_method_for_mode).copy()
            else:
                final_processed_image2 = processed_img2_intermediate
    except Exception as e:
        print(f'ERROR during resize_images_processor final processing/resizing: {e}')
        traceback.print_exc()
        return (processed_img1_intermediate, processed_img2_intermediate)
