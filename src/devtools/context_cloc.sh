...
Convenience wrapper around cloc for Improve-ImgSLI and the external sli-ui-toolkit.

Recommended (cloc tables only, full directory expansion, no size cap):

  ./launcher.sh context --cloc-only

Full context bundle (git metadata, repo trees, English help, *.md/*.txt docs + cloc):

  ./launcher.sh context

Options:
  --cloc-only                Output only cloc tables (default: cloc.txt; expands all dirs)
  -o, --output FILE          Output file (default: context.txt, or cloc.txt with --cloc-only)
  --max-lines N              Max lines copied per file in bundle mode (default: 800)
  --max-chars N              Stop bundle before output grows beyond N chars (default: 260000)
  --stats-threshold N        Expand cloc subdirs above N code lines (default: 8000; 0 = all)
  --toolkit-dir DIR          sli-ui-toolkit repository directory
  --no-stats                 Skip cloc in full bundle mode
...
TREE_IGNORE='*.git|.venv|venv|__pycache__|.pytest_cache|.mypy_cache|.ruff_cache|build|dist|Cache|blob_storage|.idea|.vscode|.codex|.agents|.claude|context.txt|app_context.txt|cloc.txt'
...
# Non-English locale directory segments (help/i18n). Keep ``en``; match ``pt_BR``.
LANG_DIR_RE='/(ar|bg|cs|da|de|el|es|et|fa|fi|fr|he|hi|hu|id|it|ja|ko|lt|nl|no|pl|pt_BR|pt|ro|ru|sk|sl|sv|th|tr|uk|zh)/'
...
collect_english_help() {
    local repo_dir="$1"
    local file repo_label rel key lines chars

    # Priority pass: English help bodies + navigation tree, before the general
    # *.md sweep eats the --max-chars budget on docs/.
    while IFS= read -r -d '' file; do
        if [[ "$file" == *.json ]]; then
            is_context_output "$file" && continue
            [[ "$file" =~ $IGNORE_DIR_RE ]] && continue
            repo_label="$(repo_label_for_path "$repo_dir")"
            rel="$(relpath "$repo_dir" "$file")"
            key="$repo_label:$rel"
            [[ -n "${seen_files[$key]:-}" ]] && continue
            [[ "$written_chars" -ge "$MAX_TOTAL_CHARS" ]] && continue
            read -r lines chars _ < <(wc -lm "$file" 2>/dev/null)
            lines=${lines:-0}
            chars=${chars:-0}
            if [[ "$written_chars" -gt 0 && $((written_chars + chars)) -gt "$MAX_TOTAL_CHARS" ]]; then
                write_line ""
                write_line "--- SKIPPED: $repo_label/$rel (would exceed --max-chars=$MAX_TOTAL_CHARS) ---"
                continue
            fi
        ...
        \( \
            ...
            -o -path '*/help/en/*' \
            -o -path '*/resources/help/tree.json' \
        \) \
        -type f \( -name '*.md' -o -name '*.txt' -o -name 'tree.json' \) \
...
cloc_target_exclude_re(){
    ...
    printf '(^|/)(%s|context\.txt|app_context\.txt|cloc\.txt)$' "$exclude_output_basename"
}
...
process_repo_cloc() {
    local repo_dir="$1"
    local stats_threshold="${2:-0}"
    local repo_label

    repo_dir="$(cd "$repo_dir" && pwd)"
    repo_label="$(repo_label_for_path "$repo_dir")"

    log_step "cloc: $repo_label"
...
process_repo() {
    ...
    write_line ""
    write_line "## $repo_label English help (priority)"
    collect_english_help "$repo_dir"
...
if [[ "$CLOC_ONLY" -eq 1 && "$OUTPUT_EXPLICIT" -eq 0 && "$OUTPUT_FILE" == "context.txt" ]]; then
    OUTPUT_FILE="cloc.txt"
fi

: >"$OUTPUT_FILE"

if [[ "$CLOC_ONLY" -eq 1 ]]; then
    log_step "Building cloc report -> $OUTPUT_FILE"
    write_line "# Improve-ImgSLI cloc report"
    write_line "Generated: $(date -Is)"
    write_line "Repositories: Improve-ImgSLI, sli-ui-toolkit (when found)"
    write_line "Directory expansion: unlimited (--stats-threshold 0)"

    process_repo_cloc "$REPO_ROOT" 0

    if [[ -n "$TOOLKIT_DIR" && -d "$TOOLKIT_DIR" ]]; then
        process_repo_cloc "$TOOLKIT_DIR" 0
    else
        write_line ""
    ...
    echo "Готово: $OUTPUT_FILE ($(wc -m <"$OUTPUT_FILE" | tr -d ' ') chars)"
    exit 0
fi
...
write_line "Collected file contents: English help first, then other *.md/*.txt"
