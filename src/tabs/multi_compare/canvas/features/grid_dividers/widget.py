"""
Widget feature definition for grid_dividers — presentation-layer contract.

Reducers/commands are still Phase A no-op scaffolding. ``build_gesture_bindings``
was added in Phase D (D3): the binding is registered here but ``canvas_widget.py``
...
from tabs.multi_compare.canvas.features.grid_dividers.input.gestures import build_grid_dividers_gesture_bindings
...
def build_widget_feature() -> CanvasWidgetFeature:
    return CanvasWidgetFeature(
        ...
        build_gesture_bindings=build_grid_dividers_gesture_bindings,
    )
