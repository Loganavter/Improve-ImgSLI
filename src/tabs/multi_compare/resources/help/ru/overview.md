...
- **Внешние дропы** — файлы из файлового менеджера, вставка из буфера обмена или **Переместить** из другой вкладки загружаются сразу в автоматически размещённые слоты с тостом загрузки; клика для размещения не нужно, подсвеченного ожидания нет.
...
Правый клик по слоту — действия для изображения, в том числе [Свойства изображения](help://image_properties) (метаданные файла и позиция слота) и **Переместить** (ghost → бросок на другую вкладку сразу загружает туда изображение с автоматическим размещением).
