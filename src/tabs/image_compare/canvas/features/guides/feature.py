...
from tabs.image_compare.canvas.features.guides.scene.objects import GuidesSceneObject
...
def _is_visible_magnifier_object(obj: CanvasSceneObject) -> bool:
    return bool(
        getattr(obj, "kind", None) == "magnifier" and getattr(obj, "visible", False)
    )
...
def build_guides_object(
    *,
    ...
    circles = (
        tuple(getattr(magnifier, "circles", ()) or ()) if magnifier is not None else ()
    )
    capture_center = (
        getattr(magnifier, "capture_center", None) if magnifier is not None else None
    )
    capture_radius = float(getattr(magnifier, "capture_radius", 0.0) or 0.0)
    show_laser = (
        bool(getattr(magnifier, "show_laser", True)) if magnifier is not None else True
    )
    guides_color = (
        getattr(magnifier, "guides_color", None) if magnifier is not None else None
    )
    if (
        magnifier is None
...
def apply_guides_object(scene, context: CanvasSceneApplyContext) -> None:
    ...
    guides_objects = [
        obj
        for obj in scene.iter_objects(kind="guides")
