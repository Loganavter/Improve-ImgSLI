...
def _scene_compare_pos(
    handler, pos: QPointF, use_internal_coords: bool = True
) -> Point:
    return handler.geometry.get_scene_compare_pos(
...
def overlay_geometry_at(
    ...
    scene_object = find_scene_object_at_position(
        scene, compare_pos, session_type="image_compare"
    )
...
def is_point_in_overlay(
    handler, pos: QPointF, use_internal_coords: bool = True
) -> bool:
    return (
        overlay_geometry_at(handler, pos, use_internal_coords=use_internal_coords)
        is not None
    )
...
def pick_overlay_at(
    ...
    match = find_scene_object_at_position(
        scene, compare_pos, session_type="image_compare"
    )
    if match is not None:
        set_active = registry().get_feature_command_by_alias("overlay.set_active_instance")
...
def apply_capture_drag(handler, cursor_pos: QPointF) -> None:
    ...
    command = registry().get_feature_command_by_alias("overlay.update_capture_drag")
...
def update_internal_split(handler, position: QPointF) -> None:
    ...
    state_q = registry().get_feature_command_by_alias("overlay.active_state")
    ...
    if float(model["internal_split"]) != clamped_val:
        command = registry().get_feature_command_by_alias(
