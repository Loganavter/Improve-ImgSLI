    ...
    *,
    ...
    for model in draw_models:
        ...
        capture_circles.append(
            CaptureCircle(
                center=QPointF(
                    cap_center_x / render_scale, cap_center_y / render_scale
                ),
    ...
    interp_mode = {
        ...
        str(interpolation_method or render.interpolation_method or "BILINEAR").upper(),
        1,
    )
...
def shift_layout_to_tile(
    ...
    *,
    ...
    return replace(
        layout,
        slots=tuple(
            replace(slot, center=shift_pt(slot.center)) for slot in layout.slots
        ),
        ...
        capture_center=(
            shift_pt(layout.capture_center)
        ...
        guide_sets=tuple(
            replace(
                ...
                target_centers=tuple(
                    shift_pt(target) for target in guide.target_centers
                ),
            )
