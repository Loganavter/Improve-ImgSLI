...
def apply_magnifier_plan_overlay(canvas, plan) -> None:
    ...
    for guide in layout.guide_sets:
        ...
        converted_guides.append(
            (
                ...
                [
                    _convert_point(target, to_wx, to_wy)
                    for target in guide.target_centers
                ],
                conv_tr,
                guide.color,
            )
    ...
    converted_overlay_centers = [
        _convert_tuple_point(cx, cy, to_wx, to_wy) for cx, cy in layout.overlay_centers
    ]
    overlay_radius = to_wr(float(layout.overlay_radius))

    effective_capture = (
        capture_center
        if (plan.capture_visible and capture_center is not None)
        else None
    )
