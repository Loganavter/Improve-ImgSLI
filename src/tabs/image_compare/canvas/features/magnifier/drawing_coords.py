...
def get_magnifier_drawing_coords(
    ...
    if bool(magnifier.visible):
        ...
        magnifier_size_pixels = max(
            10, int(round(magnifier.size_relative * target_max_dim_drawing))
        )
        ...
        offset_visual = (
            getattr(
                interaction,
                "interactive_offset_relative_visual",
        ...
        mid_x = int(
            round(base_pos_x * drawing_width + offset_visual.x * target_max_dim_drawing)
        )
        mid_y = int(
            round(
                base_pos_y * drawing_height + offset_visual.y * target_max_dim_drawing
            )
        )
