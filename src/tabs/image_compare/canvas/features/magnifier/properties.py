...
def build_magnifier_properties() -> tuple[CanvasFeatureProperty, ...]:
    ...
    divider_color = _track_descriptor(
        "magnifier.divider.color", "Divider Color", "color"
    )
    ...
    return (
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {
                "value": bool(
                    get_magnifier_widget_state(snap.viewport_state.view_state).enabled
                )
            },
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {
                "r": int(
                    get_magnifier_widget_state(
                        snap.viewport_state.view_state
                    ).default_divider_color.r
                ),
                "g": int(
                    get_magnifier_widget_state(
                        snap.viewport_state.view_state
                    ).default_divider_color.g
                ),
                "b": int(
                    get_magnifier_widget_state(
                        snap.viewport_state.view_state
                    ).default_divider_color.b
                ),
                "a": int(
                    get_magnifier_widget_state(
                        snap.viewport_state.view_state
                    ).default_divider_color.a
                ),
            },
            write_snapshot=lambda snap, ch: setattr(
                get_magnifier_widget_state(snap.viewport_state.view_state),
