    ...
    return (
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {
                "value": float(
                    _capture_size_from_view_state(snap.viewport_state.view_state)
                ),
            },
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {
                "value": bool(
                    get_capture_widget_state(snap.viewport_state.view_state).visible
                )
            },
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {
                "r": int(
                    get_capture_widget_state(snap.viewport_state.view_state).color.r
                ),
                "g": int(
                    get_capture_widget_state(snap.viewport_state.view_state).color.g
                ),
                "b": int(
                    get_capture_widget_state(snap.viewport_state.view_state).color.b
                ),
                "a": int(
                    get_capture_widget_state(snap.viewport_state.view_state).color.a
                ),
            },
            write_snapshot=lambda snap, ch: _set_snapshot_capture_state(
                snap,
...
def build_capture_state_queries():
    ...
    return (
        CanvasFeatureStateQuery(
            query_id="widget_state",
            handler=lambda view_state: get_capture_widget_state(view_state),
        ),
...
def build_capture_settings_event_bindings() -> (
...
CAPTURE_COMMAND_ALIASES = (
    ...
    CanvasFeatureCommandAlias(
        "capture.settings.toggle_visibility", "settings.toggle_visibility"
    ),
