...
def _apply_render_scene(presenter):
    ...
    image_label.set_split_position_sync(
        lambda split: _sync_split_position(presenter, split)
    )


def _sync_split_position(presenter, split_position: float):
    viewport = presenter.store.viewport
    if (
        abs(float(viewport.view_state.split_position) - float(split_position)) <= 1e-6
        and abs(
            float(viewport.view_state.split_position_visual) - float(split_position)
        )
        <= 1e-6
    ):
        return
...
def set_image_layers(
    ...
    if is_canvas_widget(presenter):
        presentation = build_live_store_presentation(presenter.store)
        img1 = presentation.display_image1
