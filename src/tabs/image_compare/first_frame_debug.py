"""``IMGSLI_IC_FIRST_FRAME_DEBUG``-gated first-frame timeline for Image Compare.

Mirror of ``multi_compare/first_frame_debug.py`` so both canvas tabs log the
same first-frame ordering: canvas construction → first show → renderer
initialize → each painted present → ``firstFrameRendered`` emission → startup
placeholder hide. Lets the two tabs' timelines be diffed directly when one
shows a blank/stale/transparent first frame and the other doesn't.
...
_ORIGIN_ATTR = "_ic_first_frame_debug_origin"
_LAST_ATTR = "_ic_first_frame_debug_last"
...
def ic_first_frame_debug_enabled() -> bool:
    return _env_flag("IMGSLI_IC_FIRST_FRAME_DEBUG")
...
def _widget_tag(widget) -> str:
    ...
    ``P`` marks the primary live page canvas (the tab sets ``_ffd_primary``);
...
def ic_first_frame_debug(widget, message: str, *args) -> None:
    ...
    unless ``IMGSLI_IC_FIRST_FRAME_DEBUG`` is set.
    """
    ...
    try:
        ...
        delta = 0.0
    logger.info(
        "[ic-first-frame] +%9.1fms (+%7.1fms)  %s  %s",
        total_ms,
        delta,
        _widget_tag(widget),
...
def ic_first_frame_readiness(widget) -> dict:
    ...
    try:
        renderer = getattr(widget, "_rhi_renderer", None)
        if renderer is not None:
            ...
            out["composition"] = (
                getattr(widget, "_active_render_plan", None) is not None
            )
    except Exception:
        pass
    return out
...
def ic_first_frame_readiness_repr(widget) -> str:
    data = ic_first_frame_readiness(widget)
    return (
...
def ic_first_frame_surface_repr(widget) -> str:
