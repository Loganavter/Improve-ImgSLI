...
class SnapshotFrameRenderer:
    ...
    def _resolve_images(self, snap, request: VideoRenderRequest):
        source1 = snap.sources[0] if len(snap.sources) > 0 else None
        source2 = snap.sources[1] if len(snap.sources) > 1 else None
        img1 = self._image_loader(source1, request.auto_crop)
        img2 = self._image_loader(source2, request.auto_crop)
        if not img1:
            img1 = Image.new(
                "RGBA",
                (
