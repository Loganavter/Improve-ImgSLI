...
class ThumbnailService(QObject):
    ...
    def _generate_single_thumbnail(
        ...
        try:
            _thlog.debug(
                "thumbnail_worker_start index=%s fps=%s thread=%s",
                index, fps, threading.current_thread().name,
            )
            ...
            _thlog.debug(
                "thumbnail_worker_end index=%s ok=%s size=%s elapsed_ms=%.1f",
                index,
                result is not None,
    ...
    def _queue_thumbnail_worker(
        ...
        *,
        ...
        _thlog.debug(
            "thumbnail_worker_queued index=%s priority=%s active_threads=%s max_threads=%s",
            index,
            priority,
