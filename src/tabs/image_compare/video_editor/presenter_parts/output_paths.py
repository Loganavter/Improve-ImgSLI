...
class OutputPathCoordinator:
    ...
    def generate_suggested_name(self) -> str:
        ...
        snap = snaps[0]
        names = snap.names or ()
        fallbacks = [f"img{i + 1}" for i in range(len(names) or 2)]
        parts = [
            self._sanitize(name if name else fallbacks[i])
            for i, name in enumerate(names or fallbacks)
        ]
        return "_".join(parts) if parts else "video_export"
