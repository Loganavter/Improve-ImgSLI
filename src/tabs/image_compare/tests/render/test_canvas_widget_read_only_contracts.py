"""Read-only CanvasWidget previews must not let viewport input mutate the scene."""
...
def _canvas():
    return SimpleNamespace(
        runtime_state=CanvasRuntimeState(_read_only=True),
        width=lambda: 100,
        height=lambda: 80,
        update=lambda: None,
...
def test_live_canvas_with_store_keeps_interactive_render_mode_without_plan():
    ...
    canvas = SimpleNamespace(
        runtime_state=CanvasRuntimeState(_store=store),
        width=lambda: 320,
        height=lambda: 180,
        objectName=lambda: "",
