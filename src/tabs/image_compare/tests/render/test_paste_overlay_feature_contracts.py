...
Dogma source: docs/dev/QRHI_CANVAS_FEATURES.md §GL passes.
"""
...
def test_qrhi_render_runs_feature_passes():
    ...
    rhi_renderer."""
    content = _read("src/tabs/image_compare/canvas/rhi_renderer/__init__.py")
...
def test_paste_overlay_pass_registered_for_qrhi():
    ...
    names = {
        type(p).__name__
        for p in get_canvas_registry("image_compare").get_render_passes()
    }
