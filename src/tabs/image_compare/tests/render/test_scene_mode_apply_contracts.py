...
Dogma source: docs/dev/QRHI_CANVAS_FEATURES.md §SceneVisibility / RenderPhase.
"""
...
def test_apply_magnifier_objects_omits_interactive_payloads_in_export(monkeypatch):
    ...
    monkeypatch.setattr(
        "tabs.image_compare.canvas.features.magnifier.scene.apply.get_magnifier_widget_state",
        lambda view_state: SimpleNamespace(intersection_highlight_enabled=True),
    )
    monkeypatch.setattr(
        "tabs.image_compare.canvas.features.magnifier.scene.apply.registry",
        lambda: SimpleNamespace(
            get_feature_command_by_alias=lambda alias: (
                lambda view_state: SimpleNamespace(color=SimpleNamespace(r=255, g=255, b=255, a=255))
            )
...
def test_apply_magnifier_objects_keeps_interactive_payloads_in_interactive(monkeypatch):
    ...
    monkeypatch.setattr(
        "tabs.image_compare.canvas.features.magnifier.scene.apply.compute_occluded_capture_arcs",
