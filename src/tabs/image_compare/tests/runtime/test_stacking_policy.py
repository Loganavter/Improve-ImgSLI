...
class TestRenderPassResolution:
    ...
    def test_every_role_resolves(self, role: CanvasStackRole):
        phase, priority = resolve_render_pass_order(role)
        assert isinstance(phase, RenderPhase)
        assert isinstance(priority, int)

    def test_underlay_before_annotation(self):
        p1, _ = resolve_render_pass_order(CanvasStackRole.UNDERLAY_SPLIT)
        p2, _ = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_FRAME)
        assert p1 < p2

    def test_frame_before_content(self):
        phase_f, pri_f = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_FRAME)
        phase_c, pri_c = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_CONTENT)
        assert phase_f == phase_c
        assert pri_f < pri_c

    def test_annotation_before_hud(self):
        p1, _ = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_CONTENT)
        p2, _ = resolve_render_pass_order(CanvasStackRole.HUD_LABEL)
        assert p1 < p2

    def test_hud_before_debug(self):
        p1, _ = resolve_render_pass_order(CanvasStackRole.HUD_LABEL)
        p2, _ = resolve_render_pass_order(CanvasStackRole.DEBUG_VIS)
        assert p1 < p2

    def test_guide_before_ring_before_content_in_same_phase(self):
        phase_g, pri_g = resolve_render_pass_order(CanvasStackRole.ANNOTATION_GUIDE)
        phase_r, pri_r = resolve_render_pass_order(CanvasStackRole.ANNOTATION_RING)
        phase_c, pri_c = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_CONTENT)
    ...
    def test_border_above_ring_and_content(self):
        phase_r, pri_r = resolve_render_pass_order(CanvasStackRole.ANNOTATION_RING)
        phase_c, pri_c = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_CONTENT)
        phase_b, pri_b = resolve_render_pass_order(CanvasStackRole.ANNOTATION_BORDER)
...
class TestCanvasRenderPassBase:
    ...
    def test_stack_role_resolution(self):
        ...
        expected_phase, expected_pri = resolve_render_pass_order(CanvasStackRole.HUD_LABEL)
...
class TestFeatureRenderPassesUseStackRole:
    ...
    def test_all_discovered_passes_have_stack_role(self):
        ...
        registry.register_package(features_pkg)
    ...
    def test_no_hardcoded_layer_priority_in_feature_passes(self):
        """Ensure no feature pass module contains raw layer= or priority= on pass classes.

        Render-pass classes don't have to live directly in ``passes.py`` (a
        feature may split its passes across several modules, e.g. magnifier's
        ``arc_passes.py``/``magnifier_pass.py``), so this scans every ``.py``
        file in the feature package that actually references
        ``CanvasRenderPass`` — not just ``passes.py``, and not unrelated
        modules like gesture bindings (which have their own, unrelated
        ``priority=``) or scene-build helpers (``stack_hint(layer=...)``).
        """
        import re
        ...
        for entry in os.scandir(features_root):
            if not entry.is_dir() or entry.name.startswith("_"):
                continue
            if not os.path.isfile(os.path.join(entry.path, "passes.py")):
                continue
            for root, dirs, files in os.walk(entry.path):
                ...
                for fname in files:
                    ...
                    with open(fpath, encoding="utf-8") as f:
                        content = f.read()
                    if "CanvasRenderPass" not in content:
                        continue
                    matches = pattern.findall(content)
                    if matches:
                        violations.append(
                            f"{entry.name}/{os.path.relpath(fpath, entry.path)}: {matches}"
                        )

        assert not violations, (
            "Feature pass modules still contain hardcoded layer/priority:\n"
            + "\n".join(violations)
        )

    def test_stack_policy_has_no_render_order_collisions(self):
        """QRHI_CANVAS_FEATURES.md: stack roles resolve to unambiguous render pass ordering."""
        seen = {}
        for role in CanvasStackRole:
            order = resolve_render_pass_order(role)
    ...
    def test_image_overlay_content_renders_before_hud_label(self):
        """QRHI_CANVAS_FEATURES.md: image annotations must stay below HUD labels."""
        content_order = resolve_render_pass_order(CanvasStackRole.IMAGE_OVERLAY_CONTENT)
        hud_order = resolve_render_pass_order(CanvasStackRole.HUD_LABEL)
