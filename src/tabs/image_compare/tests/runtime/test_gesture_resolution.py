...
Dogma source: docs/dev/QRHI_CANVAS_FEATURES.md §"Gesture Bindings (Mouse Routing)".
"""
...
def get_canvas_feature_gesture_bindings():
    return get_canvas_registry("image_compare").get_feature_gesture_bindings()
...
def fake_bindings(monkeypatch):
    ...
    def _install(bindings):
        monkeypatch.setattr(
            gr,
            "get_canvas_registry",
            lambda session_type: SimpleNamespace(
                get_feature_gesture_bindings=lambda: tuple(bindings)
            ),
        )
