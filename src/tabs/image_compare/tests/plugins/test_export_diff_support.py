...
def test_snapshot_builder_reuses_cached_diff_scene_images(monkeypatch):
    ...
    monkeypatch.setattr(
        builder_module,
        "build_export_render_scene",
        lambda *_args, **_kwargs: RenderScene(diff_mode_active=True, diff_mode_int=1),
    )
    monkeypatch.setattr(
        builder_module,
        "build_canvas_plan",
...
def test_export_render_scene_uses_main_interpolation_for_base_filter(monkeypatch):
    import tabs.image_compare.services.gpu_export_scene as scene_module

    monkeypatch.setattr(
        scene_module,
        "build_render_scene",
        lambda *_args, **_kwargs: RenderScene(zoom_interpolation_method="NEAREST"),
    )
    ...
    scene = build_export_render_scene(store, divider_thickness_export=0)

    assert scene.zoom_interpolation_method == "LANCZOS"

def test_export_render_scene_coerces_nearest_for_base_filter(monkeypatch):
    import tabs.image_compare.services.gpu_export_scene as scene_module
...
def test_image_export_frame_preserves_edge_magnifier_position_and_expands_canvas():
    ...
    # `resolve_feature_virtual_layout` looks up the canvas feature registry
    # for the active session's `session_type` (see
    # ui/canvas_infra/scene/registry.py's `get_canvas_registry`), so the
    # magnifier's layout requirement is only found once both a workspace
    # session of type "image_compare" is active and that tab type's
    # features have been registered.
    ImageCompareTab().register_canvas_features()

    store = Store()
    store.create_workspace_session(session_type="image_compare")
