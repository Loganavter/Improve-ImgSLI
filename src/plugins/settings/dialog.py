...
class SettingsDialog(QDialog):
    ...
    def _calculate_and_apply_geometry(self):
        ...
        sidebar_w = LayoutCache.get_instance().get_container_width(
            "settings_sidebar",
        ...
        required_width = sidebar_w + max_group_width + total_width_margins
    ...
    def _init_general_page(self):
        ...
        self.combo_theme.setPrecomputedLayoutKey("theme_selector", self.current_language)
    ...
    def _init_interface_page(self):
        ...
        self.combo_font_family = FluentComboBox(auto_resize_width=False)
    ...
    def _init_performance_page(self):
        ...
        self.ui_fps_group = CustomGroupWidget(self.tr("settings.ui_refresh_rate", self.current_language))
        ui_fps_layout = QHBoxLayout()
        ui_fps_layout.setContentsMargins(5, 5, 5, 5)

        self.combo_ui_fps = FluentComboBox()

        from PyQt6.QtGui import QGuiApplication
        screen = QGuiApplication.primaryScreen()
        driver_hz = int(round(screen.refreshRate())) if screen else 60

        auto_text = self.tr("settings.ui_refresh_rate_auto_detected", self.current_language).format(hz=driver_hz)
        self.combo_ui_fps.addItem(auto_text, 0)

        for fps in [30, 60, 75, 120, 144, 165, 240]:
            fps_text = self.tr("settings.ui_refresh_rate_fps", self.current_language).format(fps=fps)
            self.combo_ui_fps.addItem(fps_text, fps)

        store_obj = p.get('store', None)
        if store_obj and hasattr(store_obj, 'settings'):
            current_val = getattr(store_obj.settings, 'ui_target_fps', 0)
        else:
            current_val = p.get('current_ui_fps', 0)

        idx = self.combo_ui_fps.findData(current_val)
        if idx != -1:
            self.combo_ui_fps.setCurrentIndex(idx)
        else:
            self.combo_ui_fps.setCurrentIndex(0)

        ui_fps_layout.addWidget(self.combo_ui_fps)
        ui_fps_layout.addStretch()

        self.ui_fps_group.add_layout(ui_fps_layout)
        layout.addWidget(self.ui_fps_group)
    ...
    def get_settings(self):
        ...
        ui_fps = self.combo_ui_fps.currentData()
    ...
    def update_language(self, lang_code: str):
        ...
        from core.layout_cache import LayoutCache
        new_width = LayoutCache.get_instance().get_container_width("settings_sidebar", lang_code)
        ...
        if hasattr(self, 'combo_theme'):
            ...
            self.combo_theme.setPrecomputedLayoutKey("theme_selector", lang_code)
        ...
        if hasattr(self, 'ui_fps_group'):
            self.ui_fps_group.set_title(self.tr("settings.ui_refresh_rate", lang_code))
        if hasattr(self, 'combo_ui_fps'):
            current_ui_fps = self.combo_ui_fps.currentData()
            from PyQt6.QtGui import QGuiApplication
            screen = QGuiApplication.primaryScreen()
            driver_hz = int(round(screen.refreshRate())) if screen else 60
            self.combo_ui_fps.clear()
            auto_text = self.tr("settings.ui_refresh_rate_auto_detected", lang_code).format(hz=driver_hz)
            self.combo_ui_fps.addItem(auto_text, 0)
            for fps in [30, 60, 75, 120, 144, 165, 240]:
                fps_text = self.tr("settings.ui_refresh_rate_fps", lang_code).format(fps=fps)
                self.combo_ui_fps.addItem(fps_text, fps)
            idx_ui_fps = self.combo_ui_fps.findData(current_ui_fps)
            if idx_ui_fps != -1:
                self.combo_ui_fps.setCurrentIndex(idx_ui_fps)
            else:
                self.combo_ui_fps.setCurrentIndex(0)
