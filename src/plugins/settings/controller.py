...
from domain.qt_adapters import color_to_hex, color_to_qcolor, qcolor_to_color
...
class SettingsController(QObject):
    ...
    def set_divider_line_color(self, color: Color):
        if self.store.viewport.divider_line_color != color:
            ...
            self.settings_manager._save_setting(
                "divider_line_color", color_to_hex(color)
            )
            self.store.emit_state_change()
            if self.event_bus:
                self.event_bus.emit(CoreUpdateRequestedEvent())
    ...
    def set_magnifier_divider_color(self, color: Color):
        if self.store.viewport.magnifier_divider_color != color:
            ...
            self.settings_manager._save_setting(
                "magnifier_divider_color", color_to_hex(color)
            )
            self.store.emit_state_change()
            if self.event_bus:
                self.event_bus.emit(CoreUpdateRequestedEvent())
    ...
    def show_magnifier_divider_color_picker(self):
        dialog = QColorDialog(
            color_to_qcolor(self.store.viewport.magnifier_divider_color),
            self.presenter.main_window_app if self.presenter else None,
        )
        if dialog.exec():
            self.set_magnifier_divider_color(qcolor_to_color(dialog.selectedColor()))
            if self.event_bus:
                self.event_bus.emit(CoreUpdateRequestedEvent())
            else:
                self.update_requested.emit()
    ...
    def set_magnifier_divider_thickness(self, thickness: int):
        ...
        current_thickness = getattr(
            self.store.viewport, "magnifier_divider_thickness", -1
        )
        ...
        if current_thickness != thickness:
            ...
            self.settings_manager._save_setting(
                "magnifier_divider_thickness", thickness
            )
    ...
    def apply_smart_magnifier_colors(self):
        ...
        if dialog.exec():
            base_color_q = dialog.selectedColor()
            self.set_magnifier_divider_color(qcolor_to_color(base_color_q))
            laser_color_q = QColor(base_color_q)
            laser_color_q.setAlpha(120)
            self.set_magnifier_laser_color(qcolor_to_color(laser_color_q))
            capture_ring_color_q = QColor(base_color_q)
            capture_ring_color_q.setAlpha(230)
            self.set_capture_ring_color(qcolor_to_color(capture_ring_color_q))
    ...
    def set_magnifier_border_color(self, color: Color):
        if self.store.viewport.magnifier_border_color != color:
            ...
            self.settings_manager._save_setting(
                "magnifier_border_color", color_to_hex(color)
            )
    ...
    def set_magnifier_laser_color(self, color: Color):
        if self.store.viewport.magnifier_laser_color != color:
            ...
            self.settings_manager._save_setting(
                "magnifier_laser_color", color_to_hex(color)
            )
    ...
    def set_capture_ring_color(self, color: Color):
        if self.store.viewport.capture_ring_color != color:
            ...
            self.settings_manager._save_setting(
                "capture_ring_color", color_to_hex(color)
            )
