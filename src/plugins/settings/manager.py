...
class SettingsManager:
    ...
    def load_all_settings(self, store: Store):
        ...
        # Размеры лупы и области определения не сохраняются, всегда используются значения по умолчанию
        v.magnifier_size_relative = 0.4
        v.capture_size_relative = 0.1
        ...
        s.ui_target_fps = self._get_setting("ui_target_fps", 0, int)
    ...
    def save_all_settings(self, store: Store):
        ...
        # Размеры лупы и области определения не сохраняются
        ...
        self._save_setting("ui_target_fps", s.ui_target_fps)
    ...
    def is_first_run(self) -> bool:
        result = self._get_setting("is_first_run", True, bool)
        return result
    ...
    def reset_first_run(self):
        """Сбрасывает флаг первого запуска, чтобы снова показать onboarding"""
        logger.debug("[Onboarding Debug] reset_first_run() called - setting is_first_run to True")
        self._save_setting("is_first_run", True)
        logger.debug(f"[Onboarding Debug] After reset, is_first_run value: {self.settings.value('is_first_run')}")
