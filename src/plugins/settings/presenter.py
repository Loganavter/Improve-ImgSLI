...
class SettingsPresenter(QObject):
    ...
    def _main_window_view(self):
        return self.main_window_app
    ...
    def show_divider_color_picker(self):
        ...
        current_color = color_to_qcolor(self.store.viewport.divider_line_color)
        ...
        self.main_window_app.theme_manager.apply_theme_to_dialog(
            self._divider_color_dialog
        )
        ...
        def on_color_selected(color):
            if color.isValid():
                if self.main_controller:
                    if (
                        hasattr(self.main_controller, "event_bus")
                        and self.main_controller.event_bus
                    ):
                        self.main_controller.event_bus.emit(
                            SettingsSetDividerLineColorEvent(qcolor_to_color(color))
                        )

                main_window_view = self._main_window_view()
                if (
                    main_window_view is not None
                    and hasattr(main_window_view, "set_divider_button_color")
                ):
                    main_window_view.set_divider_button_color(color)
    ...
    def show_magnifier_divider_color_picker(self):
        ...
        if (
            self._magnifier_divider_color_dialog
            and self._magnifier_divider_color_dialog.isVisible()
        ):
            self._magnifier_divider_color_dialog.raise_()
            self._magnifier_divider_color_dialog.activateWindow()
            return

        current_color = color_to_qcolor(self.store.viewport.magnifier_divider_color)
        self._magnifier_divider_color_dialog = QColorDialog(
            current_color, self.main_window_app
        )
    ...
    def show_magnifier_border_color_picker(self):
        if (
            self._magnifier_border_color_dialog
            and self._magnifier_border_color_dialog.isVisible()
        ):
            self._magnifier_border_color_dialog.raise_()
            self._magnifier_border_color_dialog.activateWindow()
            return

        current_color = color_to_qcolor(self.store.viewport.magnifier_border_color)
        self._magnifier_border_color_dialog = QColorDialog(
            current_color, self.main_window_app
        )
        ...
        def on_color_selected(color):
            if color.isValid():
                if self.main_controller and self.main_controller.settings_ctrl:
                    self.main_controller.settings_ctrl.set_magnifier_border_color(qcolor_to_color(color))
    ...
    def show_laser_color_picker(self):
        if (
            self._magnifier_laser_color_dialog
            and self._magnifier_laser_color_dialog.isVisible()
        ):
            self._magnifier_laser_color_dialog.raise_()
            self._magnifier_laser_color_dialog.activateWindow()
            return

        current_color = color_to_qcolor(self.store.viewport.magnifier_laser_color)
        self._magnifier_laser_color_dialog = QColorDialog(
            current_color, self.main_window_app
        )
        ...
        def on_color_selected(color):
            if color.isValid():
                if self.main_controller and self.main_controller.settings_ctrl:
                    self.main_controller.settings_ctrl.set_magnifier_laser_color(qcolor_to_color(color))
    ...
    def show_capture_ring_color_picker(self):
        if (
            self._capture_ring_color_dialog
            and self._capture_ring_color_dialog.isVisible()
        ):
            self._capture_ring_color_dialog.raise_()
            self._capture_ring_color_dialog.activateWindow()
            return

        current_color = color_to_qcolor(self.store.viewport.capture_ring_color)
        self._capture_ring_color_dialog = QColorDialog(
            current_color, self.main_window_app
        )
        ...
        def on_color_selected(color):
            if color.isValid():
                if self.main_controller and self.main_controller.settings_ctrl:
                    self.main_controller.settings_ctrl.set_capture_ring_color(qcolor_to_color(color))
    ...
    def apply_smart_magnifier_colors(self):
        ...
        def on_color_selected(color):
            if color.isValid():
                ...
                settings_ctrl = (
                    self.main_controller.settings_ctrl if self.main_controller else None
                )

                if settings_ctrl:

                    settings_ctrl.set_magnifier_border_color(qcolor_to_color(border_color))
                    settings_ctrl.set_magnifier_laser_color(qcolor_to_color(laser_color))
                    settings_ctrl.set_capture_ring_color(qcolor_to_color(capture_ring_color))
    ...
    def update_interpolation_combo_box_ui(self):
        ...
        method_keys = [
            k for k in method_keys_all if k != "EWA_LANCZOS" or WAND_AVAILABLE
        ]
        ...
        if (
            main_window_view is not None
            and hasattr(main_window_view, "update_interpolation_combo_state")
        ):
            main_window_view.update_interpolation_combo_state(
    ...
    def setup_view_buttons(self):
        ...
        if (
            main_window_view is not None
            and hasattr(main_window_view, "configure_diff_mode_actions")
        ):
            main_window_view.configure_diff_mode_actions(
                diff_actions, self.store.viewport.diff_mode
            )
        ...
        if (
            main_window_view is not None
            and hasattr(main_window_view, "configure_channel_mode_actions")
        ):
            main_window_view.configure_channel_mode_actions(
                channel_actions, self.store.viewport.channel_view_mode
            )
