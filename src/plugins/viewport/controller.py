...
class ViewportController(QObject):
    ...
    def _dispatch_action(self, action, clear_caches: bool = False, clamp_pos: bool = False, skip_update: bool = False):
        if self._dispatcher:
    ...
    def toggle_magnifier(self, enabled: bool):
        ...
        self._dispatch_action(ToggleMagnifierAction(enabled), skip_update=True)

        if enabled and not self.store.viewport.active_magnifier_id:
            self._dispatch_action(SetActiveMagnifierIdAction("default"), skip_update=True)
    ...
    def _clamp_capture_position(self):
        ...
        thickness_float = thickness_factor * math.sqrt(max(1.0, float(capture_size_px)))
        thickness = max(float(AppConstants.MIN_CAPTURE_THICKNESS), min(float(AppConstants.MAX_CAPTURE_THICKNESS), thickness_float))

        safe_radius_px = (capture_size_px / 2.0) + (thickness / 2.0)

        radius_rel_x = safe_radius_px / unified_w if unified_w > 0 else 0
        radius_rel_y = safe_radius_px / unified_h if unified_h > 0 else 0
