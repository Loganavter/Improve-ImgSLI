...
class MainController(QObject):
    ...
    def toggle_magnifier(self, use_magnifier_checked: bool):
        logger.debug(
            "toggle_magnifier(use=%s) current=%s",
            use_magnifier_checked,
            getattr(self.store.viewport, "use_magnifier", None),
        )
        if self.event_bus:
        ...
        if self.store:
            ...
            logger.debug("toggle_magnifier invalidated render cache")
    ...
    def toggle_include_filenames_in_saved(self, include_checked: bool):
        logger.debug(
            "toggle_include_filenames_in_saved(include=%s) current=%s",
            include_checked,
            getattr(self.store.viewport, "include_file_names_in_saved", None),
        )
        if self.event_bus:
        ...
        if (
            ...
            if not self.store.viewport.resize_in_progress:
                ...
                logger.debug("toggle_include_filenames_in_saved: Set resize_in_progress=True")

            self.store.invalidate_render_cache()
            logger.debug("toggle_include_filenames_in_saved: invalidated render cache")
        ...
        if self.presenter:
            actual_state = getattr(self.store.viewport, "include_file_names_in_saved", False)
            self.presenter.ui.toggle_edit_layout_visibility(actual_state)
