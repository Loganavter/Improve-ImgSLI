...
class ToggleScrollableIconButton(QWidget):
    ...
    def wheelEvent(self, event):
        ...
        if old_value > 0 and new_val == 0:
            ...
            if self._saved_value is not None and self._saved_value > 0:
                ...
                new_val = min(self._max_value, max(self._min_value, 3))
    ...
    def _show_value_popup(self, val=None):
        ...
        if overlay_layer is not None:
            overlay_layer.show_popup(
                f"toggle_scrollable_icon_button:{id(self)}",
                self,
                text=popup_text,
                pixmap=pixmap,
                size=popup_size,
    ...
    def _hide_value_popup(self):
        overlay_layer = get_overlay_layer(self)
        if overlay_layer is not None:
            overlay_layer.hide_popup(f"toggle_scrollable_icon_button:{id(self)}")
    ...
    def paintEvent(self, event):
        ...
        current_icon_enum = (
            self._icon_checked if self.isChecked() else self._icon_unchecked
        )
        ...
        if self._is_hovered and not self._is_scrolling:
            ...
            self._draw_value_at(
                painter, QPoint(int(self.rect().center().x()), value_y), 9
            )
        ...
        if self._show_underline and self._underline_color is not None:
            config = UnderlineConfig(
                ...
                arc_radius=2.0,
                alpha=(
                    self._underline_color.alpha()
                    if self._underline_color.alpha() < 255
                    else 200
                ),
