...
class FluentSlider(QSlider):
    ...
    def sizeHint(self) -> QSize:
        ...
        h = max(
            base.height(),
            max(2 * self.RADIUS + 2, self.TRACK_HEIGHT + 2 * pad, fm_h + pad),
        )
    ...
    def _groove_rect(self) -> QRectF:
        ...
        return QRectF(
            self.MARGIN_H, y, max(1.0, r.width() - 2 * self.MARGIN_H), self.TRACK_HEIGHT
        )
    ...
    def paintEvent(self, _):
        ...
        if t > 0.0:
            painter.save()
            clip_rect = QRectF(
                rectf.left(), rectf.top(), rectf.width() * t, rectf.height()
            )
        ...
        painter.setBrush(QBrush(accent))
        painter.drawEllipse(
            QPointF(float(center.x()), float(center.y())), inner_r, inner_r
        )
