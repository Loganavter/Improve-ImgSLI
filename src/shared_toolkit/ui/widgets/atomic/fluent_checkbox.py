    ...
    def paintEvent(self, _):
        ...
        if is_checked or is_indeterminate:
            ...
            if is_checked:
                ...
                y2_pre = (
                    indicator_rect.top() + indicator_rect.height() * self.CHECK_Y2_PRE
                )
                x3 = indicator_rect.left() + indicator_rect.width() * self.CHECK_X3
                y3_pre = (
                    indicator_rect.top() + indicator_rect.height() * self.CHECK_Y3_PRE
                )
        ...
        if self.text():
            ...
            elided = fm.elidedText(
                self.text(), Qt.TextElideMode.ElideRight, int(text_rect_avail.width())
            )
            painter.drawText(
                text_rect_avail,
