...
class ScrollableIconButton(QPushButton):
    ...
    def __init__(
        self, icon: AppIcon, min_value: int = 1, max_value: int = 20, parent=None
    ):
    ...
    def set_range(self, min_value: int, max_value: int):
        ...
        self._current_value = max(
            self._min_value, min(self._max_value, self._current_value)
        )
    ...
    def _show_value_popup(self, value: int):
        ...
        if overlay_layer is not None:
            overlay_layer.show_popup(
                f"scrollable_icon_button:{id(self)}",
                self,
                text=str(value),
                size=popup_size,
                position="top",
                offset=10,
                timeout_ms=self._popup_timer.interval(),
            )
        else:
            ...
            popup_x = (
                button_global_pos.x() + (self.width() - self._value_popup.width()) // 2
            )
    ...
    def _hide_value_popup(self):
        overlay_layer = get_overlay_layer(self)
        if overlay_layer is not None:
            overlay_layer.hide_popup(f"scrollable_icon_button:{id(self)}")
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(
            text_rect, Qt.AlignmentFlag.AlignCenter, str(self._current_value)
        )
