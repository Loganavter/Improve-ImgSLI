...
class MinimalistScrollBar(QScrollBar):
    ...
    def _get_handle_rect(self):
        ...
        if self.orientation() == Qt.Orientation.Vertical:
            ...
            handle_pos_rel = (
                ((self.value() - self.minimum()) / scroll_range * track_len)
                if scroll_range > 0
                else 0
            )
            ...
            return QRect(
                int(handle_x), int(handle_y), int(current_thickness), int(handle_len)
            )
        else:
            ...
            return QRect(
                int(handle_x), int(handle_y), int(handle_len), int(current_thickness)
            )
    ...
    def mouseMoveEvent(self, event):
        if self._is_dragging:
            ...
            if track_len > 0:
                new_value = (
                    self.minimum() + (mouse_pos_in_track / track_len) * scroll_range
                )
                self.setValue(int(new_value))
...
class OverlayScrollArea(QScrollArea):
    def __init__(self, parent=None):
        ...
        self.verticalScrollBar().rangeChanged.connect(
            lambda *_: self._sync_steps_from_native()
        )
    ...
    def _apply_viewport_mask(self):
        ...
        rect = viewport.rect().adjusted(1, 1, -1, -1)
        if rect.isEmpty():
            return
    ...
    def _update_scrollbar_visibility(self, min_items_count=0):
        ...
        if self.widget():
            ...
            items_count = (
                min_items_count if min_items_count > 0 else self._stored_items_count
            )
