...
class ColorOptionsFlyout(BaseFlyout):
    ...
    def __init__(self, parent=None, current_language: str = "en", store=None):
        ...
        self.btn_capture.clicked.connect(
            lambda: self.colorOptionClicked.emit("capture")
        )
        ...
        self.btn_border = SimpleIconButton(
            AppIcon.MAGNIFIER_BORDER_COLOR, self.container
        )
        ...
        self.btn_divider.setToolTip(
            tr("ui.choose_magnifier_divider_line_color", self.current_language)
        )
        self.btn_divider.clicked.connect(
            lambda: self.colorOptionClicked.emit("divider")
        )
