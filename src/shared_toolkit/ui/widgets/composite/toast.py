...
class ToastNotification(QWidget):
    ...
    def show_message(
        ...
        if "\n" in message:
            lines = message.split("\n")
            lines_elided = [
                font_metrics.elidedText(line, Qt.TextElideMode.ElideRight, max_width)
                for line in lines
            ]
            self.message_label.setText("\n".join(lines_elided))
        else:
            elided_text = font_metrics.elidedText(
                message, Qt.TextElideMode.ElideRight, max_width
            )
    ...
    def update_message(
        self, new_message: str, max_width: int, success: bool, duration: int = 4000
    ):
        font_metrics = QFontMetrics(self.message_label.font())
        if "\n" in new_message:
            ...
            elided_text = font_metrics.elidedText(
                new_message, Qt.TextElideMode.ElideRight, max_width
            )
            self.message_label.setText(elided_text)
        self.adjustSize()
        if duration > 0:
...
class ToastManager(QObject):
    ...
    def update_toast(
        self, toast_id: int, new_message: str, success: bool, duration: int = 4000
    ):
