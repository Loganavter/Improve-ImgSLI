...
def register_canvas_gl_pass_feature_package(package: ModuleType) -> None:
...
def get_canvas_gl_render_passes() -> tuple[CanvasGLRenderPass, ...]:
    ...
    Auto-discovers ``GL_RENDER_PASSES`` lists from registered tab feature
    packages. Features that do not have a ``gl_passes.py`` are silently
    skipped.
    """
        ...
        for module_info in sorted(
            ...
            try:
                module = importlib.import_module(
                    f"{features_pkg.__name__}.{module_info.name}.gl_passes"
                )
