...
class MainWindow(QWidget):
    ...
    def initialize_application(self):
        ...
        is_first_run_value = self.settings_manager.is_first_run()

        if _ONBOARDING_OVERLAY_AVAILABLE and is_first_run_value:
    ...
    def _on_onboarding_completed(self, mode_key: str):
        logger.debug(f"[Onboarding Debug] _on_onboarding_completed() called with mode_key: {mode_key}")
        ...
        if self.presenter and self.presenter.main_controller:
            ...
            if layout_plugin and layout_plugin.manager:
                # Устанавливаем presenter в manager, если его еще нет
                if not layout_plugin.manager.presenter:
                    layout_plugin.manager.presenter = self.presenter
        ...
        if hasattr(self, 'onboarding_overlay') and self.onboarding_overlay:
            logger.debug("[Onboarding Debug] Hiding and deleting onboarding overlay")
            self.onboarding_overlay.hide()
            self.onboarding_overlay.deleteLater()
        else:
            logger.debug("[Onboarding Debug] No onboarding_overlay to hide")
        self.onboarding_overlay = None
        logger.debug("[Onboarding Debug] _on_onboarding_completed() finished")
