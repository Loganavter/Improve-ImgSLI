...
class RatingGestureTransaction:
		...
		if delta > 0:
			for _ in range(delta):
				if self.main_controller and self.main_controller.session_ctrl:
					self.main_controller.session_ctrl.increment_rating(self.image_number, self.item_index)
		elif delta < 0:
			for _ in range(-delta):
				if self.main_controller and self.main_controller.session_ctrl:
					self.main_controller.session_ctrl.decrement_rating(self.image_number, self.item_index)
	...
	def rollback(self) -> None:
		...
		if self._accumulated_delta != 0:
			...
			if self.main_controller and self.main_controller.session_ctrl:
				self.main_controller.session_ctrl.set_rating(self.image_number, self.item_index, self.starting_score)
