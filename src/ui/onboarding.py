...
class OnboardingOverlay(QWidget):
    ...
    def __init__(self, settings_manager, store, parent=None):
        logger.debug("[Onboarding Debug] OnboardingOverlay.__init__() called")
        super().__init__(parent)
        self.settings_manager = settings_manager
        self.store = store
        self.theme_manager = ThemeManager.get_instance()
        self._current_index = 0
        logger.debug(f"[Onboarding Debug] OnboardingOverlay created, parent: {parent}, parent type: {type(parent)}")
    ...
    def _apply_theme(self):
        """Применяет тему к onboarding overlay через QSS файлы"""
        # Используем метод ThemeManager для применения QSS с заменой плейсхолдеров
        self.theme_manager.apply_theme_to_widget(self)
        
        # Обновляем цвета элементов, которые используют цвета темы
        self._update_theme_colors()

    def _update_theme_colors(self):
        """Обновляет цвета элементов, которые динамически используют цвета темы"""
        # Обновляем состояние кнопок режимов
        if hasattr(self, '_current_index') and hasattr(self, 'mode_buttons'):
            self._update_state(self._current_index)
        
        # Обновляем цвета текста в подсказках и описаниях
        if hasattr(self, 'hint_label'):
            text_col = self.theme_manager.get_color("WindowText").name()
            hint_font_size = getattr(self, '_hint_font_size', 15)
            self.hint_label.setStyleSheet(
                f"color: {text_col}; font-size: {hint_font_size}px; font-weight: 600; opacity: 0.8;"
            )
        
        # Обновляем цвета в слайдах
        if hasattr(self, 'stack'):
            for i in range(self.stack.count()):
                page = self.stack.widget(i)
                if page:
                    for child in page.findChildren(QLabel):
                        if child.text() and child.text() in [mode.get("desc", "") for mode in self.modes]:
                            text_col = self.theme_manager.get_color("dialog.text").name()
                            child.setStyleSheet(f"font-size: 16px; color: {text_col}; margin-top: 15px; opacity: 0.9;")

    def _finish(self):
        logger.debug("[Onboarding Debug] OnboardingOverlay._finish() called")
        key = self.modes[self._current_index]["key"]
        logger.debug(f"[Onboarding Debug] Selected mode key: {key}")
        if self.settings_manager:
            logger.debug("[Onboarding Debug] Saving ui_mode and calling set_first_run_completed()")
            self.settings_manager._save_setting("ui_mode", key)
            self.settings_manager.set_first_run_completed()
        else:
            logger.warning("[Onboarding Debug] settings_manager is None!")
        logger.debug(f"[Onboarding Debug] Emitting completed signal with key: {key}")
