...
class UIManager(QObject):
    ...
    def _show_magnifier_visibility_flyout(self, reason: str = "hover"):
        ...
        if hasattr(self.ui, 'btn_magnifier') and self.ui.btn_magnifier is not None:
            ...
            if reason == "wheel":
                self.magnifier_visibility_flyout.schedule_auto_hide(100)
            else:
    ...
    def show_settings_dialog(self):
        ...
        if self._settings_dialog is None:
            self._settings_dialog = SettingsDialog(
                ...
                current_ui_fps=getattr(self.store.settings, 'ui_target_fps', 0),
                store=self.store,
            )
    ...
    def _apply_settings(self, settings):
        (
            ...
            new_auto_psnr, new_auto_ssim, new_auto_crop, new_ui_mode, new_video_fps, new_ui_fps
        ) = settings
        ...
        if getattr(self.store.settings, "ui_target_fps", 0) != new_ui_fps:
            self.store.settings.ui_target_fps = new_ui_fps
            if self.main_controller is not None and self.main_controller.settings_manager is not None:
                self.main_controller.settings_manager._save_setting("ui_target_fps", new_ui_fps)

            if self.main_controller is not None and hasattr(self.main_controller, 'presenter'):
                presenter = getattr(self.main_controller.presenter, 'image_canvas_presenter', None)
                if presenter and hasattr(presenter, '_update_scheduler_timer'):
                    ...
                    presenter._update_scheduler_timer.setInterval(interval)
