...
logger = logging.getLogger("ImproveImgSLI.MainWindowUI")
...
class Ui_ImageComparisonApp:
    def setupUi(self, main_window: QWidget):
        ...
        self._unblock_timer = None
        self._pending_unblock_on_resize = False
    ...
    def toggle_edit_layout_visibility(self, checked: bool):
        ...
        prev_resize = None
        if hasattr(self, 'main_window') and hasattr(self.main_window, 'store'):
            prev_resize = self.main_window.store.viewport.resize_in_progress
            if not prev_resize:
                self.main_window.store.viewport.resize_in_progress = True
                should_unblock = True

        if should_unblock and self.image_label:
            self.image_label._block_paint = True

        self.edit_layout_widget.setVisible(bool(checked))
        self.edit_layout_widget.updateGeometry()

        if hasattr(self, 'main_window') and self.main_window.layout():
            self.main_window.layout().invalidate()
        ...
        if should_unblock:
            ...
            if self._unblock_timer is not None and self._unblock_timer.isActive():
                self._unblock_timer.stop()
                logger.debug("toggle_edit_layout_visibility: cancelled previous timer")

            self._unblock_timer = QTimer()
            self._unblock_timer.setSingleShot(True)
            self._unblock_timer.timeout.connect(self._unblock_render)
            self._unblock_timer.start(0)
    ...
    def toggle_magnifier_panel_visibility(self, visible: bool):
        ...
        if self.image_container_layout:
            self.image_container_layout.invalidate()
            self.image_container_layout.activate()
        ...
        if should_unblock:
            ...
            if self._unblock_timer is not None and self._unblock_timer.isActive():
                self._unblock_timer.stop()
                logger.debug("toggle_magnifier_panel_visibility: cancelled previous timer")
    ...
    def _unblock_render(self):
        ...
        if self.image_label:
            self.image_label._block_paint = False

        if hasattr(self, 'main_window') and hasattr(self.main_window, 'store'):
            prev_state = self.main_window.store.viewport.resize_in_progress

            if prev_state:
                self.main_window.store.viewport.resize_in_progress = False

                if hasattr(self.main_window, 'schedule_update'):
                    self.main_window.schedule_update()
            else:
                logger.debug("_unblock_render() skipping - flag already False")
    ...
    def update_rating_display(
        ...
        if hasattr(self, 'main_window') and hasattr(self.main_window, 'store'):
            if image_number == 1:
                has_images = bool(self.main_window.store.document.image_list1)
            else:
                has_images = bool(self.main_window.store.document.image_list2)
