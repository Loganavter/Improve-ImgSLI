...
from ui.canvas_features.filename_overlay.events import SettingsToggleIncludeFilenamesInSavedEvent
...
from .config import GLFilenameOverlayConfig

def reduce_filename_overlay_render_config(config: RenderConfig, action: Action) -> RenderConfig:
...
def build_filename_overlay_properties() -> tuple[CanvasFeatureProperty, ...]:
    ...
    enum_channels = (ChannelDescriptor("value", "Value", "enum", interpolate_values=False),)
    color_channels = (
    ...
    return (
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {"value": bool(snap.viewport_state.render_config.include_file_names_in_saved)},
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {"value": float(snap.viewport_state.render_config.font_size_percent)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
                "font_size_percent",
        ...
        CanvasFeatureProperty(
            ...
            serialize_setting=lambda ch: int(ch["value"]),
            deserialize_setting=lambda raw: {"value": int(float(raw))},
            read_snapshot=lambda snap: {"value": float(snap.viewport_state.render_config.font_weight)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
                "font_weight",
        ...
        CanvasFeatureProperty(
            ...
            serialize_setting=lambda ch: int(ch["value"]),
            deserialize_setting=lambda raw: {"value": int(float(raw))},
            read_snapshot=lambda snap: {"value": float(snap.viewport_state.render_config.text_alpha_percent)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
                "text_alpha_percent",
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {"value": bool(snap.viewport_state.render_config.draw_text_background)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
        ...
        CanvasFeatureProperty(
            ...
            read_snapshot=lambda snap: {"value": str(snap.viewport_state.render_config.text_placement_mode)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
                "text_placement_mode",
                str(ch["value"]),
        ...
        CanvasFeatureProperty(
            ...
            serialize_setting=lambda ch: int(ch["value"]),
            deserialize_setting=lambda raw: {"value": int(float(raw))},
            read_snapshot=lambda snap: {"value": float(snap.viewport_state.render_config.max_name_length)},
            write_snapshot=lambda snap, ch: setattr(
                snap.viewport_state.render_config,
                "max_name_length",
                max(1, int(float(ch["value"]))),
...
def build_filename_overlay_toolbar_bindings() -> tuple[CanvasFeatureToolbarBinding, ...]:
    return (
        CanvasFeatureToolbarBinding(
...
def build_filename_overlay_settings_event_bindings() -> tuple[CanvasFeatureSettingsEventBinding, ...]:
    return (
        CanvasFeatureSettingsEventBinding(
...
def build_filename_overlay_render_scene_overrides(store) -> dict:
    ...
    if viewport is None:
        return {"filename_overlay": GLFilenameOverlayConfig()}
...
FILENAME_OVERLAY_COMMAND_ALIASES = (
    ...
    CanvasFeatureCommandAlias("labels.settings.set_font_size", "settings.set_font_size"),
    CanvasFeatureCommandAlias("labels.settings.set_font_weight", "settings.set_font_weight"),
    CanvasFeatureCommandAlias("labels.settings.set_text_color", "settings.set_text_color"),
    CanvasFeatureCommandAlias("labels.settings.set_background_color", "settings.set_background_color"),
    CanvasFeatureCommandAlias("labels.settings.set_draw_background", "settings.set_draw_background"),
    CanvasFeatureCommandAlias("labels.settings.set_placement_mode", "settings.set_placement_mode"),
    CanvasFeatureCommandAlias("labels.settings.set_text_alpha", "settings.set_text_alpha"),
)
