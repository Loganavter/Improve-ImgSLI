...
def command_viewport_set_smoothing_interpolation_method(store, method: str) -> None:
    dispatcher = getattr(store, "_dispatcher", None)
    if dispatcher is not None:
        dispatcher.dispatch(SetGuidesSmoothingInterpolationMethodAction(method), scope="viewport")
    else:
        state = get_guides_widget_state(store.viewport.view_state)
...
def build_guides_commands() -> dict[str, Any]:
    return {
        ...
        "query.enabled": lambda view_state: bool(get_guides_widget_state(view_state).enabled),
        "query.thickness": lambda view_state: int(get_guides_widget_state(view_state).thickness),
