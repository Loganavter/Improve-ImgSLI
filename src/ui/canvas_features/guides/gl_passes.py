...
class GuidesPass(CanvasRenderPass):
    ...
    def prepare(self, widget, ctx, resource_updates) -> None:
        ...
        for capture_center, capture_radius, target_centers, target_radii, color in tuple(
            widget.runtime_state._guide_sets
        ):
            ...
            end_radius = max(
                0.0, float(capture_radius or 0.0) * float(ctx.zoom_level)
            )
