...
def build_magnifier_object(
    *,
    ...
    is_combined = bool(model.visible_left and model.visible_right and effective_spacing <= THRESHOLD + 1e-5)

    if is_combined:
        if is_visual_diff and model.visible_center:
            if not model.is_horizontal:
                if model.visible_center:
                    circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y - mag_radius - 4), mag_radius, "diff"))
                if model.visible_left or model.visible_right:
                    circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y + mag_radius + 4), mag_radius, "combined"))
            else:
                if model.visible_center:
                    circles.append(MagnifierCircle(Point(base_mag_x - mag_radius - 4, base_mag_y), mag_radius, "diff"))
                if model.visible_left or model.visible_right:
                    circles.append(MagnifierCircle(Point(base_mag_x + mag_radius + 4, base_mag_y), mag_radius, "combined"))
        elif model.visible_left or model.visible_right:
            role = "combined" if model.visible_left and model.visible_right else ("left" if model.visible_left else "right")
            circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y), mag_radius, role))
    else:
        spacing_px = effective_spacing * target_max_dim
        dist = mag_radius + (spacing_px / 2.0)
        if not model.is_horizontal:
            left_circle = MagnifierCircle(Point(base_mag_x - dist, base_mag_y), mag_radius, "left")
            right_circle = MagnifierCircle(Point(base_mag_x + dist, base_mag_y), mag_radius, "right")
        else:
            left_circle = MagnifierCircle(Point(base_mag_x, base_mag_y - dist), mag_radius, "left")
            right_circle = MagnifierCircle(Point(base_mag_x, base_mag_y + dist), mag_radius, "right")

        if is_visual_diff:
            offset_3 = max(mag_radius * 2.0, mag_radius * 2.0 + spacing_px)
            if not model.is_horizontal:
                if model.visible_left:
                    circles.append(MagnifierCircle(Point(base_mag_x - offset_3, base_mag_y), mag_radius, "left"))
                if model.visible_right:
                    circles.append(MagnifierCircle(Point(base_mag_x + offset_3, base_mag_y), mag_radius, "right"))
            else:
                if model.visible_left:
                    circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y - offset_3), mag_radius, "left"))
                if model.visible_right:
                    circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y + offset_3), mag_radius, "right"))
            if model.visible_center:
                circles.append(MagnifierCircle(Point(base_mag_x, base_mag_y), mag_radius, "center"))
        else:
            if model.visible_left:
    ...
    return MagnifierSceneObject(
        ...
        stack_hint=MAGNIFIER_Z_ORDER.stack_hint(
            layer=CanvasStackLayer.OBJECT_ACTIVE if is_active else CanvasStackLayer.OBJECT,
            priority=z_index,
        ),
        ...
        for other in visible_magnifiers:
            for circle in getattr(other, "circles", ()) or ():
                ...
                if distance + radius <= oradius + 1e-6:
                    ranges = [(0.0, 360.0)]
                    break
                cosine = (distance * distance + radius * radius - oradius * oradius) / (2.0 * distance * radius)
                cosine = max(-1.0, min(1.0, cosine))
                half_angle = math.degrees(math.acos(cosine))

                center_angle = math.degrees(math.atan2(-dy, dx))
                ranges.extend(_normalize_arc_deg(center_angle - half_angle, center_angle + half_angle))
            if ranges == [(0.0, 360.0)]:
                break
...
def apply_magnifier_objects(scene, context: CanvasSceneApplyContext) -> None:
        ...
        for obj in all_magnifiers:
            ...
            if center is not None and radius > 0:
                hidden_capture_circles.append(
                    (QPointF(center.x, center.y), radius, obj.id == scene.active_object_id)
                )
    ...
    capture_center = None
    capture_radius = 0.0
    if active_magnifier.capture_center is not None and active_magnifier.capture_radius > 0:
