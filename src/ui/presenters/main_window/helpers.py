...
def open_image_dialog(presenter, image_number: int):
    ...
    if paths:
        ...
        QTimer.singleShot(
            delay,
            lambda: (
                presenter.main_controller.session_ctrl.load_images_from_paths(
                    paths, image_number
                )
                if presenter.main_controller is not None
                and presenter.main_controller.session_ctrl is not None
                else None
            ),
        )
...
def start_interactive_movement(presenter):
    if hasattr(presenter, "image_canvas_presenter"):
...
def on_magnifier_guides_toggled(presenter, checked: bool):
    presenter.main_controller.toggle_magnifier_guides(checked)

def on_magnifier_guides_thickness_changed(presenter, thickness: int):
    presenter.main_controller.set_magnifier_guides_thickness(thickness)

def on_magnifier_element_hovered(presenter, element_name: str):
    if not presenter.store:
        return
    presenter.store.viewport.highlighted_magnifier_element = element_name
...
def on_magnifier_element_hover_ended(presenter):
    if not presenter.store:
        return
    presenter.store.viewport.highlighted_magnifier_element = None
    presenter.store.emit_state_change()
    if presenter.event_bus:
        presenter.event_bus.emit(CoreUpdateRequestedEvent())
    else:
...
def on_color_option_clicked(presenter, option: str):
    if option == "divider":
        presenter.settings_presenter.show_magnifier_divider_color_picker()
    elif option == "capture":
        presenter.settings_presenter.show_capture_ring_color_picker()
    elif option == "border":
        presenter.settings_presenter.show_magnifier_border_color_picker()
    elif option == "laser":
        presenter.settings_presenter.show_laser_color_picker()
