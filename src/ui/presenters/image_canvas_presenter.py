...
class ImageCanvasPresenter(QObject):
    ...
    def __init__(
        ...
        self._last_padding_left: int = 0
        self._last_padding_top: int = 0
        self._cached_clean_pixmap: QPixmap | None = None

        self._cached_pixmap_1: QPixmap | None = None
        self._cached_pixmap_2: QPixmap | None = None

        self._pending_interactive_mode: bool | None = None

        self._is_rendering_active = False
        self._pending_update_queued = False
        ...
        user_fps = getattr(self.store.settings, 'ui_target_fps', 0)

        if user_fps > 0:

            effective_fps = user_fps
        else:

            screen = QGuiApplication.primaryScreen()

            driver_rate = int(round(screen.refreshRate())) if screen else 60

            effective_fps = driver_rate if driver_rate > 20 else 60

        effective_fps = max(30, min(360, effective_fps))
    ...
    def update_minimum_window_size(self):
        ...
        try:
            ...
            if current_min.width() != new_min_w or current_min.height() != new_min_h:
                ...
                # Автоматически изменяем размер окна, если текущая ширина меньше новой минимальной
                current_width = self.main_window_app.width()
                current_height = self.main_window_app.height()
                if current_width < new_min_w or current_height < new_min_h:
                    new_width = max(current_width, new_min_w)
                    new_height = max(current_height, new_min_h)
                    self.main_window_app.resize(new_width, new_height)
        finally:
    ...
    def update_comparison_if_needed(self) -> bool:
        ...
        if self.store.viewport.resize_in_progress:
            logger.debug("update_comparison_if_needed: BLOCKED by resize_in_progress")
            return False
        ...
        if self._is_rendering_active:
            self._pending_update_queued = True
            return False
        ...
        is_dragging_split = self.store.viewport.is_dragging_split_line
        use_magnifier = self.store.viewport.use_magnifier

        has_cached_layers = (
            getattr(self, '_cached_pixmap_1', None) is not None and
            getattr(self, '_cached_pixmap_2', None) is not None and
            not self._cached_pixmap_1.isNull() and
            not self._cached_pixmap_2.isNull()
        )

        size_valid = False
        if has_cached_layers:
            if (self._cached_pixmap_1.width() == self.store.viewport.pixmap_width and
                self._cached_pixmap_1.height() == self.store.viewport.pixmap_height):
                size_valid = True

        if is_dragging_split and not use_magnifier and has_cached_layers and size_valid:

            target_rect = self.store.viewport.image_display_rect_on_label
            if not target_rect.isEmpty():
                split_rel = self.store.viewport.split_position_visual
                is_horizontal = self.store.viewport.is_horizontal

                if not is_horizontal:
                    divider_pos = int(target_rect.left() + target_rect.width() * split_rel)
                else:
                    divider_pos = int(target_rect.top() + target_rect.height() * split_rel)

                ui_data = self.ui.image_label._ui_data.copy()
                ui_data["divider_pos"] = divider_pos
                ui_data["divider_visible"] = self.store.viewport.divider_line_visible
                ui_data["divider_horizontal"] = is_horizontal
                ui_data["divider_color"] = self.store.viewport.divider_line_color
                ui_data["divider_thickness"] = self.store.viewport.divider_line_thickness
                ui_data["img_rect"] = target_rect

                self.ui.image_label.set_ui_overlays(ui_data)
                return True
        ...
        if (not self.store.viewport.image1 or not self.store.viewport.image2 or
            ...
            if cache_key in cache:
                ...
                self._cached_pixmap_1 = None
                self._cached_pixmap_2 = None
        ...
        if not cache_is_valid:
            ...
            try:
                ...
                if img1_scaled:
                    self._cached_pixmap_1 = pil_to_qpixmap_optimized(img1_scaled, copy=False)
                if img2_scaled:
                    self._cached_pixmap_2 = pil_to_qpixmap_optimized(img2_scaled, copy=False)

                use_magnifier = self.store.viewport.use_magnifier
                if not use_magnifier:
                    self.ui.image_label.set_layers(self._cached_pixmap_1, self._cached_pixmap_2)
                else:

                    self.ui.image_label.set_layers(None, None)
            ...
            use_magnifier = self.store.viewport.use_magnifier
            if not use_magnifier:
                if self.ui.image_label._pixmap_left is None or self.ui.image_label._pixmap_right is None:

                    if (getattr(self, '_cached_pixmap_1', None) is not None and
                        getattr(self, '_cached_pixmap_2', None) is not None):
                        self.ui.image_label.set_layers(self._cached_pixmap_1, self._cached_pixmap_2)
                    else:

                        from shared.image_processing.qt_conversion import pil_to_qpixmap_optimized
                        if self.store.viewport.scaled_image1_for_display:
                            self._cached_pixmap_1 = pil_to_qpixmap_optimized(self.store.viewport.scaled_image1_for_display, copy=False)
                        if self.store.viewport.scaled_image2_for_display:
                            self._cached_pixmap_2 = pil_to_qpixmap_optimized(self.store.viewport.scaled_image2_for_display, copy=False)
                        self.ui.image_label.set_layers(self._cached_pixmap_1, self._cached_pixmap_2)
            else:
        ...
        self._is_rendering_active = True
        self._pending_update_queued = False
    ...
    def _on_display_scaling_ready(self, result):
        ...
        from shared.image_processing.qt_conversion import pil_to_qpixmap_optimized
        if img1_s:
            self._cached_pixmap_1 = pil_to_qpixmap_optimized(img1_s, copy=False)
        if img2_s:
            self._cached_pixmap_2 = pil_to_qpixmap_optimized(img2_s, copy=False)
    ...
    def _on_worker_finished(self, result_payload: dict, params: dict, finished_task_id: int):
        ...
        label_dims = params.get("label_dims", (0, 0))
        use_magnifier = params.get("render_params_dict", {}).get("use_magnifier", False)
        is_interactive = result_payload.get("is_interactive", False)
        ...
        if result_payload.get("is_interactive"):
            use_magnifier = self.store.viewport.use_magnifier
            target_rect = self.store.viewport.image_display_rect_on_label

            if not use_magnifier:

                if (self.ui.image_label._pixmap_left is None or
                    self.ui.image_label._pixmap_right is None):

                    from shared.image_processing.qt_conversion import pil_to_qpixmap_optimized
                    if self.store.viewport.scaled_image1_for_display:
                        self._cached_pixmap_1 = pil_to_qpixmap_optimized(
                            self.store.viewport.scaled_image1_for_display, copy=False)
                    if self.store.viewport.scaled_image2_for_display:
                        self._cached_pixmap_2 = pil_to_qpixmap_optimized(
                            self.store.viewport.scaled_image2_for_display, copy=False)
                    if (getattr(self, '_cached_pixmap_1', None) and
                        getattr(self, '_cached_pixmap_2', None)):
                        self.ui.image_label.set_layers(self._cached_pixmap_1, self._cached_pixmap_2)
            else:

                base_pil = result_payload.get("base_image")

                if base_pil:
                    clean_pix = pil_to_qpixmap_optimized(base_pil, copy=False)

                    label_width, label_height = params.get("label_dims", self.get_current_label_dimensions())

                    if (self._cached_clean_pixmap and
                        self._cached_clean_pixmap.width() == label_width and
                        self._cached_clean_pixmap.height() == label_height):
                        final_bg = self._cached_clean_pixmap
                        final_bg.fill(Qt.GlobalColor.transparent)
                    else:
                        final_bg = QPixmap(label_width, label_height)
                        final_bg.fill(Qt.GlobalColor.transparent)

                    painter = QPainter(final_bg)
                    painter.drawPixmap(target_rect.x(), target_rect.y(), clean_pix)
                    painter.end()

                    self._cached_clean_pixmap = final_bg
                    self.ui.image_label.set_background_pixmap(final_bg)

                elif self._cached_clean_pixmap:
                    if self.ui.image_label._background_pixmap != self._cached_clean_pixmap:
                        self.ui.image_label.set_background_pixmap(self._cached_clean_pixmap)

            overlay_pil = result_payload.get("overlay_image")
            overlay_pos = result_payload.get("overlay_pos")

            if overlay_pil:
                overlay_pix = pil_to_qpixmap_optimized(overlay_pil, copy=False)

                final_x = target_rect.x() + overlay_pos.x()
                final_y = target_rect.y() + overlay_pos.y()

                self.ui.image_label.set_overlay_pixmap(overlay_pix, QPoint(final_x, final_y))

                center_x = final_x + overlay_pix.width() // 2
                center_y = final_y + overlay_pix.height() // 2

                self.store.viewport.magnifier_screen_center = QPoint(center_x, center_y)

                self.store.viewport.magnifier_screen_size = overlay_pix.width()

            else:
                self.ui.image_label.set_overlay_pixmap(None)

            ui_data = result_payload.get("ui_data", {})
            offset = target_rect.topLeft()

            if ui_data.get("capture_rect"):
                ui_data["capture_rect"] = ui_data["capture_rect"].translated(offset)

            if ui_data.get("divider_visible"):
                if not ui_data.get("divider_horizontal"):
                    ui_data["divider_pos"] += offset.x()
                else:
                    ui_data["divider_pos"] += offset.y()
                if ui_data.get("img_rect"):
                    ui_data["img_rect"] = ui_data["img_rect"].translated(offset)

            lasers = ui_data.get("lasers", [])
            new_lasers = []
            for p1, p2, col, w in lasers:
                new_lasers.append((p1 + offset, p2 + offset, col, w))
            ui_data["lasers"] = new_lasers

            self.ui.image_label.set_ui_overlays(ui_data)

        else:

            final_pil = result_payload.get("final_canvas")
            if final_pil:
                pix = pil_to_qpixmap_optimized(final_pil, copy=False)
                ...
                pad_l = result_payload.get("padding_left", 0)
                pad_t = result_payload.get("padding_top", 0)

                painter.drawPixmap(target_rect.x() - pad_l, target_rect.y() - pad_t, pix)
                painter.end()

                logger.debug(f"_on_worker_finished: setting final background pixmap size={final_bg.size()}")
                self.ui.image_label.set_background_pixmap(final_bg)

                self.ui.image_label.set_overlay_pixmap(None)
                self.ui.image_label.set_ui_overlays({})

                self._cached_base_pixmap = final_bg
                ...
                if magnifier_bbox and isinstance(magnifier_bbox, QRect):
                    ...
                    if combined_center and isinstance(combined_center, QPoint):
                        draw_start_x = target_rect.x() - pad_l
                        draw_start_y = target_rect.y() - pad_t

                        self.store.viewport.magnifier_screen_center = combined_center + QPoint(draw_start_x, draw_start_y)

                        self.store.viewport.magnifier_screen_size = max(magnifier_bbox.width(), magnifier_bbox.height())
                    else:
                        draw_x = target_rect.x() - pad_l
                        draw_y = target_rect.y() - pad_t
                ...
                self.current_displayed_pixmap = final_bg

        if self._pending_update_queued:
            self._pending_update_queued = False
