...
class MainWindowPresenter(QObject):
    def __init__(
        ...
        self._last_magnifier_state = None
        self._last_text_edit_state = None
        ...
        try:
            ...
            if self.main_controller is not None and self.main_controller.plugin_coordinator is not None:
                ...
                if layout_plugin is not None:
                    layout_plugin.setup_ui_reference(self.ui, self)
    ...
    def _apply_initial_settings_to_ui(self):
        ...
        if hasattr(self.ui.btn_orientation, 'set_color'):
            ...
            div_visible = self.store.viewport.divider_line_visible and self.store.viewport.divider_line_thickness > 0
            main_color = self.store.viewport.divider_line_color if div_visible else None
            self.ui.btn_orientation.set_color(main_color)

        if hasattr(self.ui.btn_magnifier_orientation, 'set_color'):
            mag_div_visible = self.store.viewport.magnifier_divider_visible and self.store.viewport.magnifier_divider_thickness > 0
            mag_color = self.store.viewport.magnifier_divider_color if mag_div_visible else None
            self.ui.btn_magnifier_orientation.set_color(mag_color)

        if hasattr(self.ui, 'btn_magnifier_color_settings'):

            if hasattr(self.ui.btn_magnifier_color_settings, '_update_underline_colors'):
                self.ui.btn_magnifier_color_settings._update_underline_colors()
        ...
        current_text_edit = self.store.viewport.include_file_names_in_saved
        if self._last_text_edit_state != current_text_edit:
            self._last_text_edit_state = current_text_edit
            self.ui.toggle_edit_layout_visibility(current_text_edit)

        current_magnifier = self.store.viewport.use_magnifier
        if self._last_magnifier_state != current_magnifier:
            self._last_magnifier_state = current_magnifier
            self.ui.toggle_magnifier_panel_visibility(current_magnifier)
