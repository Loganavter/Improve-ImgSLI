...
from ui.widgets.glass_hud_parts import target_watch, text_mask
from ui.widgets.glass_panel_display import create_glass_panel_display_widget
...
def _caller_str(depth: int = 6) -> str:
    ...
    frames = [f for f in frames if "glass_hud.py" not in f.filename][-depth:]
...
class GlassHUD(BaseFlyout):
    ...
    def __init__(self, parent: QWidget):
        ...
        # default (see ui/widgets/glass_panel_display.py's
    ...
    def _rebuild_text_mask(self) -> bool:
        """See ``glass_hud_parts.text_mask.rebuild_text_mask`` -- kept as a
    ...
    def _maybe_update_text_mask(self) -> None:
        """See ``glass_hud_parts.text_mask.maybe_update_text_mask``."""
        text_mask.maybe_update_text_mask(self)

    def _debug_frame_submitted(self) -> None:
        """See ``glass_hud_parts.target_watch.debug_frame_submitted`` --
    ...
    def _connect_zoom_refresh(self, target_widget: QWidget) -> None:
        """See ``glass_hud_parts.target_watch.connect_zoom_refresh``."""
        target_watch.connect_zoom_refresh(self, target_widget)

    def _watch_target(self, target_widget: QWidget | None) -> None:
        """See ``glass_hud_parts.target_watch.watch_target``. Must be
