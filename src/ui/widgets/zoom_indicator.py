...
from ui.widgets.glass_hud import GlassHUD, _caller_str
...
class ZoomIndicator(GlassHUD):
    ...
    ``ui/widgets/info_hud.py`` and the shared ``ui/widgets/glass_hud.py``):
    ...
    def update_zoom(self, zoom: float, pan_x: float = 0.0, pan_y: float = 0.0):
        ...
        flyout_debug(
            ...
            pan_y,
            was_visible,
            _caller_str() if flyout_debug_enabled() else "",
        )
    ...
    def _position(self) -> None:
        ...
        x = anchor_rect.right() - w - _MARGIN
        y = anchor_rect.top() + _MARGIN
        ...
        flyout_debug(
            ...
            self._label.parentWidget().geometry() if self._label.parentWidget() else None,
