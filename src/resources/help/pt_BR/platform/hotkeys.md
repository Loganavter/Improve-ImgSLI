## Atalhos de Teclado

Combinações padrão abaixo. Remapeie a maioria dos atalhos de ação em {{tr:menu.settings}} → {{tr:settings.keyboard}}. Os movimentos `WASD` e `Space` do canvas permanecem fixos e não podem ser remapeados.

### Descubra primeiro {#discover}

Pressione `Ctrl+Shift+P` e digite um nome antes de memorizar uma lista. Execute a ação a partir da paleta, ou abra {{tr:action.palette.learn_more}} quando precisar do tópico ilustrado. F1 pode destacar a ação correspondente quando o foco está marcado.

### Plataforma {#platform}

- `Ctrl+,` — Configurações
- `Ctrl+F1` — Ajuda
- `Ctrl+N` — Seletor de sessão / nova sessão
- `Ctrl+Shift+P` — {{tr:menu.find_action}}
- `Ctrl+V` — Colar imagem
- `Ctrl+Q` — Sair

Os rótulos exatos seguem o seu pacote de idioma; remapeie em Configurações → Teclado → grupo de plataforma.

### {{tr:workspace.session_types.image_compare}} {#image-compare}

- `M` / `F` — Lupa / congelar
- `N` / `D` — Rótulos de nome de arquivo / visibilidade do divisor
- `H` / `C` — Modo de diferença / modo de canal
- `X` — Trocar
- `R` / `Ctrl+E` — Gravar / Editor de Vídeo
- `Ctrl+S` / `Ctrl+Shift+S` — Salvamento rápido / Diálogo de salvar
- `WASD` / `QE` / `Space` — Mover lupa / espaçamento / pré-visualização de lado (fixos)

Detalhes: [Comparação](help://comparison), [Lupa](help://magnifier), [Editor de Vídeo](help://video).

### {{tr:workspace.session_types.multi_compare}} {#multi-compare}

- `Ctrl+O` — Adicionar imagens (quando vinculado)
- `D` — Visibilidade da grade
- `Ctrl+S` / `Ctrl+Shift+S` — Salvamento rápido / Salvar
- `Esc` — Sair do foco do slot

Veja [{{tr:workspace.session_types.multi_compare}}](help://multi_compare).

### Janela do editor de vídeo {#video-editor}

Enquanto o editor está aberto: `Space` reproduz/pausa, `Ctrl+Z` / `Ctrl+Y` desfazer/refazer, `Delete` / `Backspace` remove a seleção. Fluxo completo de codificação: [Editor de Vídeo](help://video).
