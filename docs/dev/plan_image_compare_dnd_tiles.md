...
| 3b | 2026-09-01 | **Phase 3b is_new fix — долгосрочное решение.** `slot.py:165 is_new` теперь одна `store.transact([...])` вместо 2×`batch_changes`+5 `dispatch` (причина 1.49с на холодном `drop` в пустую сессию, лог `38.709→40.202`). `ClearImageSlotDataAction`+`SetImageSessionImage(null)`+`SetCachedDiff` в одной транзакции, `diff_service.invalidate()` вне транз. Последующие `drop` уже <5мс, теперь и первый <5мс. |
| 4 | — | Phase 4 native canvas — not started (optional, not needed после canvas-only) |
