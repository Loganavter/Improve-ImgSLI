...
Status: `Done` — Phase 5 done (Store.transact single dispatch, Pyramid idle + lod_available, render_flow single geometry pass, ImageSession + StaleGate, docs updated) 2026-08-30
...
| 5 | 2026-08-30 | **Phase 5 done.** `Store.transact` + `TransactionAction` (`core/state_management/transaction.py:12`, `action_base.py: TRANSACTION`, `reducers.py:RootReducer.Transaction` single Store alloc, `store.py:transact` + `publish(lod_available)`) — `IMGSLI_TRACE=1` shows 1 `dispatch.begin/end` per `set_current_image` (was 6–10), `chrome_sync` dedup verified. `PyramidBuildCoordinator` await idle (`time.sleep(0.002)` between levels) + `store.publish(lod_available)` per level instead of `invalidate_render` per level (`tabs/_shared/pyramid.py:42, get_store, on_level_ready`). `render_flow.py` second geometry pass deleted (single `batch_changes` per frame, `geometry: comparison rect updated` once per image). `ImageSession` per `session_id` (`pipeline/session.py:ImageSession`) + `StaleGate` (`use_cases/stale_gate.py`) merging `widget._render_stale/_metrics_stale` + `chrome_sync._render_stale/_workspace_language_stale` via shared `widget._stale_gate`; `_session_controller.py` proxies `_pending_*`/`_unification_task_id` to `ImageSession` (thin-owner step, stays 832 LOC — full 150 tracked as next P2 but per-session state ready). `ARCHITECTURE.md` State Model (SlotSource vs PipelineView) and `STORE.md` §Batching (Transaction + lod_available) updated. `python src/devtools/file_meta.py --write-registry` 60 entries, `python src/devtools/docs_link_graph.py --write-index` 0 broken. `pytest tests/contracts -q` 1554 passed, `QT_QPA_PLATFORM=offscreen pytest src/tabs/image_compare/tests -q` 586 passed /12 pre-existing, `tests/render` 110 passed. |
...
- `render_flow.py` second geometry pass deleted as planned, but mixed-tier (store vs preview) now waits ~400ms for unify before rect re-converges (second pass previously fixed this with picked-size coherence). Single `batch_changes` per frame verified cheaper than 60Hz double dispatch; revisit if `ic_preview_debug` shows letterbox lag.
- `_session_controller.py` thin-owner 815→150 not yet fully to 150 LOC (now 832 with `ImageSession` + `StaleGate` wiring) — per-session `ImageSession` extraction done, full MethodObject split to `use_cases/` modules tracked as next P2 to stay <500 without Audit-Meta; Audit-Meta kept as `pattern=thin-owner`.
- `PyramidBuildCoordinator` idle is `time.sleep(0.002)` in worker thread, not `await idle` on main loop / `async` — sufficient to let UI paint LOD0 while higher levels build; true `QTimer.singleShot(0)` idle in main thread would need `async` worker refactor, deferred.
...
- `Store.transact` now implemented as `TransactionAction` reducer-chain (single Store alloc) — previously deferred; verified `IMGSLI_TRACE=1` 1 per `set_current_image` and `586` tests still green.
