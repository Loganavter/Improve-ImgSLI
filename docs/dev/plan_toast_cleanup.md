...
Status: `Done` — A+B merged+verified on `feat/toast-cleanup` 2026-09-05 (−606 LOC, 15+38 leak/close tests green); paired-hang roots fixed+verified live (`_pop_alias`, always-map); `[toast-debug]` removed; C ready as toolkit branch `feat/toast-lifecycle` (4.2.4, unmerged — needs PR per toolkit rules). Full story: [toast_stuck_session_report.md](./toast_stuck_session_report.md)
