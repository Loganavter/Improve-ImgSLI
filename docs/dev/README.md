...
- **[KNOWN_BUGS.md](KNOWN_BUGS.md)** — Root-caused Qt/platform quirks worth remembering before re-diagnosing them
- **[rendering/display-image-pipeline.md](rendering/display-image-pipeline.md)** — unify → display-cache → render pipeline; single-writer/single-picker rule for `display_cache_image1/2`
- **[rendering/tile-rendering-system.md](rendering/tile-rendering-system.md)** — GPU tile residency for large sources, plus the host-side memory bounding (`_texture_upload_cache` LRU budget, `LazyPixelSource` memmap spill) that resolved the 18k+ px OOM freezes
...
- **Custom PySide6/QRhi rendering pipeline**
