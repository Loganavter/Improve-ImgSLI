...
class _MenuItem(QWidget):
    ...
    def set_current(self, is_current: bool):
        """Устанавливает статус выбранного элемента"""
        self._is_current = is_current
        if is_current:
            check_icon = get_app_icon(AppIcon.CHECK)
            self._check_icon = check_icon.pixmap(20, 20)
...
class _DropdownMenu(QWidget):
    ...
    def set_actions(self, actions: list[tuple[str, any]]):
        """Устанавливает действия для меню"""
    ...
    def show_at(self, pos: QPoint):
        """Показывает меню в указанной позиции с анимацией"""
...
class ToolButtonWithMenu(QWidget):
    ...
    Полностью кастомный виджет на основе QWidget.
    """
    ...
    def show_menu(self):
        """Показывает меню под кнопкой с отступом"""
        if not self._actions:
            return
