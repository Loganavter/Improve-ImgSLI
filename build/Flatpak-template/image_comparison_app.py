...
class ImageComparisonApp(QWidget):
    ...
    def _load_settings(self):
        ...
        def get_setting(key, default, target_type):
            ...
            try:
                if target_type == int: return int(value) if isinstance(value, (int, str, float)) else default
                    ...
                    try:
                         if color_val.startswith('#') and (len(color_val) == 9 or len(color_val) == 7 or len(color_val) == 4):
                             qcol = QColor(color_val)
                             if qcol.isValid(): return qcol
                    except: pass
                    print(f"Warning: Invalid color string '{color_val}' for setting '{key}'. Using default.")
                    return default
                     ...
                     try:
                         ...
                         print(f"Warning: Could not convert value '{value}' to QByteArray for setting '{key}'. Using default.")
                         return default
                return value
        ...
        saved_lang_raw = self.settings.value("language", None)
        saved_lang = None
        if isinstance(saved_lang_raw, str): saved_lang = saved_lang_raw
        elif isinstance(saved_lang_raw, bytes): saved_lang = saved_lang_raw.decode('utf-8', errors='ignore')
        elif isinstance(saved_lang_raw, QByteArray): saved_lang = bytes(saved_lang_raw).decode('utf-8', errors='ignore')
        ...
        self.loaded_filename_color = get_setting("filename_color", default_color, QColor)
        if not isinstance(self.loaded_filename_color, QColor) or not self.loaded_filename_color.isValid():
             print(f"Warning: Loaded filename color was invalid. Using default red.")
             self.loaded_filename_color = default_color
    ...
    def _init_state(self):
        ...
        self.file_name_color = self.loaded_filename_color if isinstance(self.loaded_filename_color, QColor) and self.loaded_filename_color.isValid() else QColor(255, 0, 0, 255)
    ...
    def _create_combobox_layout(self):
        ...
        self.combo_image1.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.combo_image2.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
    ...
    def _create_checkbox_layout(self):
        ...
        layout.addStretch()
        layout.addWidget(self.lang_en)
        layout.addWidget(self.lang_ru)
        layout.addWidget(self.lang_zh)
    ...
    def _create_language_checkboxes(self):
        ...
        size = QSize(24, 16)
        style = 'QCheckBox { padding: 2px; border: none; } QCheckBox::indicator { width: 24px; height: 16px; }'
    ...
    def _create_slider_layout(self):
        ...
        self.slider_size = QSlider(Qt.Orientation.Horizontal, minimum=50, maximum=400, value=self.magnifier_size)
        self.label_capture_size = QLabel()
        self.slider_capture = QSlider(Qt.Orientation.Horizontal, minimum=10, maximum=500, value=self.capture_size)
        self.label_movement_speed = QLabel()
        self.slider_speed = QSlider(Qt.Orientation.Horizontal, minimum=10, maximum=500, value=self.movement_speed_per_sec)
        self.slider_size.setToolTip(f"{self.magnifier_size} {tr('px', self.current_language)}")
        self.slider_capture.setToolTip(f"{self.capture_size} {tr('px', self.current_language)}")
        self.slider_speed.setToolTip(f"{self.movement_speed_per_sec} {tr('px/sec', self.current_language)}")
    ...
    def _create_image_label(self):
        ...
        self.image_label.setStyleSheet("QLabel { background-color: #f0f0f0; border: 1px solid grey; }")
    ...
    def _create_file_names_layout(self):
        ...
        layout.addWidget(self.file_name_label1)
        layout.addStretch()
        layout.addWidget(self.file_name_label2)
        return layout
    ...
    def _create_color_wheel_icon(self, size: QSize) -> QIcon:
        ...
        for i in range(num_segments):
            ...
            start_angle = int((i * angle_step - 90) * 16)
    ...
    def on_mouse_press(self, event):
        ...
        if not self.image_label.pixmap() or self.image_label.pixmap().isNull(): return
        pos = event.position()
    ...
    def changeEvent(self, event):
        ...
        if event.type() == QEvent.Type.WindowStateChange:
            ...
            is_maximized_or_fullscreen = bool(new_state & (Qt.WindowState.WindowMaximized | Qt.WindowState.WindowFullScreen))
            was_maximized_or_fullscreen = self.previous_geometry is not None

            if is_maximized_or_fullscreen and not was_maximized_or_fullscreen:
                self.previous_geometry = self.saveGeometry()
            elif not is_maximized_or_fullscreen and was_maximized_or_fullscreen:
                if self.previous_geometry:
                    try:
                        restored_ok = self.restoreGeometry(self.previous_geometry)
                        if not restored_ok:
                            print("Warning: restoreGeometry returned false during state change.")
                        self.previous_geometry = None
                        QTimer.singleShot(50, self.update_comparison_if_needed)
                        QTimer.singleShot(60, self._ensure_minimum_size_after_restore)
                    except Exception as e:
                        print(f"Error restoring previous geometry during state change: {e}")
                        self.previous_geometry = None
                else:
                    print("Warning: Window state changed to normal, but no previous_geometry was stored.")
                    QTimer.singleShot(50, self.update_comparison_if_needed)
                    QTimer.singleShot(60, self._ensure_minimum_size_after_restore)
    ...
    def closeEvent(self, event):
        ...
        try:
            obsolete_keys = [
                ...
                "magnifier_spacing", "magnifier_size", "capture_size"
            ]
            for key in obsolete_keys:
                if self.settings.contains(key):
                    self.settings.remove(key)
        except Exception as e:
            print(f"Error removing obsolete settings on close: {e}")
        ...
        if self.image1 and self.image2:
            try:
                ...
                QMessageBox.critical(self, tr("Error", self.current_language),
                                     f"{tr('Failed to update comparison view:', self.current_language)}\n{e}")
        elif not self.original_image1 or not self.original_image2:
             self.image_label.clear()
             self.result_image = None
             self.image1 = None
             self.image2 = None
             self.image_label.setText(tr("Load images in both slots", self.current_language))
             self.image_label.setStyleSheet("QLabel { background-color: #f0f0f0; border: 1px solid grey; color: grey; }")
        else:
            print("Warning: update_comparison called but resized images (self.image1/self.image2) are missing.")
            self.image_label.clear()
            self.result_image = None
            self.image_label.setText(tr("Error resizing images", self.current_language))
            self.image_label.setStyleSheet("QLabel { background-color: #f0f0f0; border: 1px solid grey; color: red; }")
    ...
    def update_comparison_if_needed(self):
        ...
        if self.original_image1 and self.original_image2:
            ...
            if needs_resize:
                 ...
                 if not self.image1 or not self.image2:
                      print("Error: Resizing failed, cannot update comparison.")
                      self.image_label.clear()
                      self.result_image = None
                      self.image_label.setText(tr("Error resizing images", self.current_language))
                      self.image_label.setStyleSheet("QLabel { background-color: #f0f0f0; border: 1px solid grey; color: red; }")
    ...
    def load_image(self, image_number):
        file_names, selected_filter = QFileDialog.getOpenFileNames(
            ...
            "",
            tr("Image Files", self.current_language) + " (*.png *.jpg *.jpeg *.bmp *.webp *.tif *.tiff);;" + \
    ...
    def _load_images_from_paths(self, items_to_process, image_number):
        ...
        for item in items_to_process:
            ...
            if isinstance(item, str):
                ...
                load_errors.append(f"{str(item)}: " + tr("Unexpected item type", self.current_language))
                continue
            if not path_to_open:
                print(f"  Skipping empty path string.")
                load_errors.append(tr("Empty file path received", self.current_language))
                continue
            if any(entry[1] == path_to_open for entry in target_list if len(entry) > 1 and entry[1]):
                 continue
            if path_to_open:
                try:
                    ...
                    error_detail = tr("Could not read or process image file.", self.current_language) + f" ({type(e).__name__})"
                    load_errors.append(f"{original_path_for_display}: {error_detail}")
        if loaded_count > 0:
            self._update_combobox(image_number)
            if newly_added_indices:
                new_index_to_select = newly_added_indices[-1]
                current_cb_index = combobox.currentIndex()
                needs_manual_set = (current_cb_index != new_index_to_select)
                if needs_manual_set:
                    combobox.blockSignals(True)
                    combobox.setCurrentIndex(new_index_to_select)
                    combobox.blockSignals(False)
                self._on_combobox_changed(image_number, new_index_to_select)
        ...
        if load_errors:
             ...
             QMessageBox.warning(self, tr("Error Loading Images", self.current_language),
                                  tr("Some images could not be loaded:", self.current_language) + f"\n\n - {error_list_str}")
    ...
    def _set_current_image(self, image_number, trigger_update=True):
        ...
        if 0 <= current_index < len(target_list):
            try:
                ...
                if isinstance(new_pil_img, Image.Image):
                    reset_image = False
                else:
                    print(f"Error: Item at index {current_index} for image {image_number} is not a PIL Image.")
                    reset_image = True
            except IndexError:
        ...
        if reset_image:
            ...
            if image_number == 1:
                self.original_image1 = new_pil_img; self.image1_path = new_path
                self.image1 = None
            else:
                self.original_image2 = new_pil_img; self.image2_path = new_path
                self.image2 = None
            if edit_name_widget:
        ...
        old_max_w, old_max_h = 0, 0
        new_max_w, new_max_h = 0, 0
        ...
        for i, entry in enumerate(target_list):
            ...
            try:
                if len(entry) >= 3 and isinstance(entry[2], str):
                    ...
                    display_name = os.path.basename(entry[1] or "") or f"Image {i+1}"
                else:
                    display_name = f"Image {i+1}"
            except Exception as e:
                print(f"Error getting display name for combobox item {i} in list {image_number}: {e}")
                display_name = f"Error Item {i+1}"
            max_cb_len = 60
            cb_name = (display_name[:max_cb_len-3] + "...") if len(display_name) > max_cb_len else display_name
            combobox.addItem(cb_name, userData=i)
        new_index_to_set = -1
        if 0 <= current_index < len(target_list):
            new_index_to_set = current_index
        if new_index_to_set != -1:
            combobox.setCurrentIndex(new_index_to_set)
        else:
            combobox.setCurrentIndex(-1)
        if image_number == 1:
            if self.current_index1 != new_index_to_set:
                self.current_index1 = new_index_to_set
        else:
            if self.current_index2 != new_index_to_set:
                self.current_index2 = new_index_to_set
        combobox.blockSignals(False)

    def _on_combobox_changed(self, image_number, cb_index):
        combobox = self.combo_image1 if image_number == 1 else self.combo_image2
        target_list = self.image_list1 if image_number == 1 else self.image_list2
        current_internal_index = self.current_index1 if image_number == 1 else self.current_index2
        new_internal_index = -1
        if 0 <= cb_index < combobox.count():
            user_data = combobox.itemData(cb_index)
            if isinstance(user_data, int) and 0 <= user_data < len(target_list):
                new_internal_index = user_data
            else:
                print(f"Warning: Invalid or missing userData for combobox index {cb_index} in slot {image_number}. Falling back to cb_index.")
                if 0 <= cb_index < len(target_list):
                    new_internal_index = cb_index
                else:
                    print(f"Error: Combobox index {cb_index} is out of bounds for target list {image_number} (size {len(target_list)}).")
                    new_internal_index = -1
        if new_internal_index != current_internal_index:
            if image_number == 1:
                self.current_index1 = new_internal_index
            else:
                self.current_index2 = new_internal_index
    ...
    def _on_edit_name_changed(self):
        ...
        if 0 <= current_index < len(target_list):
            ...
            if not new_name:
                 print(f"Warning: Empty name entered for image {image_number}, reverting.")
                 try:
                 ...
                 except Exception as e: print(f"Error reverting name: {e}")
                 return
            try:
    ...
    def swap_images(self):
        ...
        if hasattr(self, 'combo_image1'): self.combo_image1.blockSignals(True)
        if hasattr(self, 'combo_image2'): self.combo_image2.blockSignals(True)
        if hasattr(self, 'edit_name1'): self.edit_name1.blockSignals(True)
        if hasattr(self, 'edit_name2'): self.edit_name2.blockSignals(True)
        try:
            self._update_combobox(1)
            self._update_combobox(2)
            self._set_current_image(1, trigger_update=False)
            ...
            if hasattr(self, 'combo_image1'): self.combo_image1.blockSignals(False)
            if hasattr(self, 'combo_image2'): self.combo_image2.blockSignals(False)
            if hasattr(self, 'edit_name1'): self.edit_name1.blockSignals(False)
            if hasattr(self, 'edit_name2'): self.edit_name2.blockSignals(False)
    ...
    def _save_result_with_error_handling(self):
        try:
            ...
            if not self.image1 or not self.image2:
                 ...
                 if not self.image1 or not self.image2:
                     QMessageBox.warning(self, tr("Warning", self.current_language),
                                         tr("Resized images not available. Cannot save result. Please reload or select images.", self.current_language))
                     return
            if not self.result_image:
                print("Result image missing, attempting update before saving...")
                self.update_comparison()
                if not self.result_image:
                    QMessageBox.warning(self, tr("Warning", self.current_language),
                                        tr("Failed to generate the comparison view. Cannot save result.", self.current_language))
                    return
            ...
            traceback.print_exc()
            QMessageBox.critical(self, tr("Error", self.current_language),
                                 f"{tr('Failed to save image:', self.current_language)}\n{str(e)}")
    ...
    def _update_magnifier_position_by_keys(self):
        ...
        if self.active_keys:
            ...
            raw_ds = ds_dir * spacing_speed_per_sec * delta_time_sec
        ...
        if self.magnifier_offset_pixels != new_offset_pixels:
        ...
        if not self.active_keys:
            ...
            if pos_is_close and spacing_is_close:
                ...
                if self.magnifier_offset_pixels != final_offset_pixels:
                    self.magnifier_offset_pixels = final_offset_pixels
                    needs_final_redraw = True
                if self.magnifier_spacing != final_spacing_int_clamped:
    ...
    def toggle_freeze_magnifier(self, state):
        ...
        if new_freeze_state:
            if self.use_magnifier and self.original_image1 and self.original_image2:
                current_freeze_state = self.freeze_magnifier
                self.freeze_magnifier = False
                magnifier_pos_result_current = None
                cap_orig1 = None
                ...
                if result_width > 0 and result_height > 0:
                    try:
                        cap_orig1, _, magnifier_pos_result_current = get_original_coords(self)
                         ...
                         print(f"Error getting coordinates from get_original_coords: {e}")
                         cap_orig1 = None
                         magnifier_pos_result_current = None
                else:
                    print("Warning: Cannot get magnifier coordinates, result_image has invalid size.")
                if magnifier_pos_result_current and isinstance(magnifier_pos_result_current, QPoint) and result_width > 0 and result_height > 0:
                    ...
                    self.magnifier_spacing = round(self._magnifier_spacing_float_visual)
                     ...
                     print("Warning: Could not get valid magnifier coordinates to freeze. Aborting freeze.")
                     self.frozen_magnifier_position_relative = None
                     self.freeze_magnifier = False
                     if hasattr(self, 'freeze_button'):
    ...
    def _unfreeze_magnifier_logic(self):
        ...
        try:
            ...
            if frozen_pos_rel and current_result_width > 0 and current_result_height > 0 and \
               current_scaled_width > 0 and current_scaled_height > 0:
    ...
    def _show_help_dialog(self):
        help_text = (
            ...
            f"- {tr('Use Add buttons or dropdowns to load/select images.', self.current_language)}\n"
            f"- {tr('Use dropdowns to select from loaded images.', self.current_language)}\n"
            f"- {tr('Click and drag the split line (when magnifier is off).', self.current_language)}\n"
            f"- {tr('Check Magnifier checkbox to enable zoom.', self.current_language)}\n"
            f"- {tr('Magnifier: Click/drag on image sets the zoom capture point.', self.current_language)}\n"
            f"- {tr('Magnifier: Use WASD keys to move the zoomed view offset.', self.current_language)}\n"
            f"- {tr('Magnifier: Use QE keys to adjust spacing between capture point and zoomed view.', self.current_language)}\n"
            f"- {tr('Freeze Magnifier checkbox locks the zoomed area position relative to the image.', self.current_language)}\n"
            f"- {tr('Include file names checkbox saves names onto the output image.', self.current_language)}\n"
            f"- {tr('When Include... is checked, edit names/font size/color below.', self.current_language)}\n"
            f"- {tr('Use sliders to adjust magnifier/capture size and move speed.', self.current_language)}\n"
            f"- {tr('Swap button (⇄) swaps the images/lists between the two slots.', self.current_language)}\n"
            f"- {tr('Click language flags (top right) to change language.', self.current_language)}\n"
            f"- {tr('Save Result button outputs the current comparison view.', self.current_language)}"
        )
    ...
    def _update_split_or_capture_position(self, cursor_pos_f: QPointF):
        ...
        rel_x = pixmap_x / self.pixmap_width if self.pixmap_width > 0 else 0.0
        rel_y = pixmap_y / self.pixmap_height if self.pixmap_height > 0 else 0.0
    ...
    def update_file_names(self):
        ...
        if 0 <= self.current_index1 < len(self.image_list1):
             try:
                 _, path1, display_name1 = self.image_list1[self.current_index1]
                 edited_name1 = self.edit_name1.text().strip() if hasattr(self, 'edit_name1') else ""
                 name1_raw = edited_name1 or display_name1 or os.path.basename(path1 or "")
             except IndexError: pass
             except Exception as e: print(f"Error getting name 1 info: {e}")
        name1_raw = name1_raw or (tr("Image 1", self.current_language) if self.current_index1 == -1 else "")
        name2_raw = ""
        if 0 <= self.current_index2 < len(self.image_list2):
             try:
                 _, path2, display_name2 = self.image_list2[self.current_index2]
                 edited_name2 = self.edit_name2.text().strip() if hasattr(self, 'edit_name2') else ""
                 name2_raw = edited_name2 or display_name2 or os.path.basename(path2 or "")
             except IndexError: pass
             except Exception as e: print(f"Error getting name 2 info: {e}")
        name2_raw = name2_raw or (tr("Image 2", self.current_language) if self.current_index2 == -1 else "")
        max_len_ui = self.max_name_length
        display_name1_ui = (name1_raw[:max_len_ui-3]+"...") if len(name1_raw) > max_len_ui else name1_raw
        display_name2_ui = (name2_raw[:max_len_ui-3]+"...") if len(name2_raw) > max_len_ui else name2_raw
        if hasattr(self, 'file_name_label1') and hasattr(self, 'file_name_label2'):
            ...
            self.file_name_label1.setText(f"{prefix1}: {display_name1_ui}")
            self.file_name_label2.setText(f"{prefix2}: {display_name2_ui}")
        ...
        if name1 is None or name2 is None:
             ...
             if 0 <= self.current_index1 < len(self.image_list1):
                  try:
                      ...
                      edited_name1 = self.edit_name1.text().strip() if hasattr(self, 'edit_name1') else ""
                      name1_raw = edited_name1 or dn1
                  except IndexError: pass
                  except Exception as e: print(f"Error getting name 1 for length check: {e}")
             name1 = name1_raw or ""
             name2_raw = ""
             if 0 <= self.current_index2 < len(self.image_list2):
                  try:
                      ...
                      edited_name2 = self.edit_name2.text().strip() if hasattr(self, 'edit_name2') else ""
                      name2_raw = edited_name2 or dn2
                  except IndexError: pass
                  except Exception as e: print(f"Error getting name 2 for length check: {e}")
             name2 = name2_raw or ""
    ...
    def _edit_length_dialog(self, event):
        ...
        min_lim, max_lim = 10, 100
        new_limit, ok = QInputDialog.getInt(
            self,
            tr("Edit Length Limit", self.current_language),
            tr("Enter new maximum length for UI display ({min_lim}-{max_lim}):", self.current_language).format(min_lim=min_lim, max_lim=max_lim),
            value=current_limit,
            min=min_lim,
            max=max_lim
        )
        if ok and new_limit != current_limit:
            print(f"Changing max name length from {current_limit} to {new_limit}")
    ...
    def update_minimum_window_size(self):
        ...
        try:
            ...
            abs_min_w = 400
            abs_min_h = 350
            new_min_w = max(abs_min_w, layout_min_size.width())
            new_min_h = max(abs_min_h, layout_min_size.height())
