...
def test_snapshot_render_plan_export_preview_parity_matrix(
    ...
    def _build_export_gl_scene(store, *_args, **_kwargs):
    ...
    monkeypatch.setattr(builder_module, "build_export_gl_scene", _build_export_gl_scene)
    ...
    assert plan.gl_scene.zoom_interpolation_method == "LANCZOS"

    if diff_mode == "off":
        assert plan.gl_scene.diff_mode_int == 0
