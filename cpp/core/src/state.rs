...
//!   [`crate::plan::TextureId`]. Anything Rust needs to know about an image is
...
impl Default for GeometryState {
    fn default() -> Self {
        Self {
            pixmap_width: 0,
            pixmap_height: 0,
            image_display_rect_on_label: Rect::default(),
            fixed_label_width: None,
            fixed_label_height: None,
            active_overlay_screen_center: Point::default(),
            active_overlay_screen_size: 0,
            loaded_geometry: Vec::new(),
            loaded_was_maximized: false,
            loaded_previous_geometry: Vec::new(),
            loaded_debug_mode_enabled: false,
        }
    }
}
...
/// Mirrors `WorkspaceSession` (subset — we keep only the fields a reducer
/// touches; richer session blueprint logic stays in the orchestration layer).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Default)]
#[serde(default)]
